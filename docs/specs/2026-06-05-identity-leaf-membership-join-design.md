...
> ## ⚑ Phase-0 resolutions (2026-06-15) — these SUPERSEDE the open items below; full rationale in `docs/superpowers/specs/2026-06-15-join-e2e-phase0.md`
> **E2E crypto (§2):** AEAD nonce = repurpose the DATA `MAC` trailer → a **CRYPTED-gated 8-B cleartext seed** (4-B-zero when not CRYPTED → s18 byte-identical), `nonce = BLAKE2b(rand8 ‖ ctr ‖ dst_key_hash32)[:24]` — **cleartext inputs only** (`source_hash` is sealed). KDF `key = BLAKE2b("MR-E2E-v1" ‖ shared ‖ min(hash) ‖ max(hash))[:32]` (domain-first); AEAD **AAD** = the cleartext routing header. Replay = **accept-none v1** (honest-node). FS = **none** (static-static), accepted. No-pubkey-known = **FAIL LOUD** (never silent cleartext). Pubkey resolution: H `WANT_PUBKEY` flag → DATA TYPE **`H_ANSWER_PUBKEY`** `[target_layer][node_id][ed_pub 32]` (redundant key_hash32 dropped); **cache-on-pass keyed by FULL pubkey**, confidence-tiered like `id_bind` (resolves the 32-bit-collision worry). *(Replaces the §7.1/P3 `WANT_PUBKEY` text that referenced the now-deleted payload-flags byte.)*
> **Leaf membership (§3–§5):** `config_hash = BLAKE2b(u16 allowed_sf_bitmap LE ‖ u32 duty_ppm LE ‖ u8 name_len ‖ leaf_name)[:4]` — hash the **bitmap** (drop §3.4's "ordered SF list, do NOT sort"; the bitmap == the ascending set `sf_index` uses), quantize duty→ppm, `leaf_name` is a NEW u8-len field. `CONFIG_PULL` = a Q subtype answered by a routed DATA TYPE **`CONFIG_ANSWER`**. Epoch bump = sync max-seen-from-beacons → `max+1`; ties → higher `key_hash32` (loser pulls, no bump-war). `lineage_id` minted by a `leaf create` cfg op (two operators "creating leaf N" → two lineages = two leaves, by design).
> **BCN +10-B leaf header = wire FLAG-DAY** (reflash-all) + **s18 re-baseline** (Phase 2 = semantic parity, not byte-identical). **⚠ TODO (Phase 2): re-examine BCN truncation** — header+schedule+entries+seen-bitmap+ext can exceed `beacon_max_bytes`=151; the leaf header (pre-entries) must NEVER be cut; pin the overflow priority (entries spill first today — what about the seen-bitmap / ext?).
> **§A drift:** the DAD tiebreak is **KEY-ONLY** (not `claim_epoch→key`); the §5.5 beacon-guard is **DONE** — see the node-id spec's Phase-0 block.
> **Sequencing:** Phase 1 = E2E crypto (s18 byte-identical, CRYPTED off-default); Phase 2 = leaf membership (flag-day). Independent; **E2E first**.
