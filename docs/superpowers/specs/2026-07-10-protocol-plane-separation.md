...
## 11. Addendum — 2026-07-11/12 mobile-DM + team fixes (this arc; plane-separation properties)
All below are `is_mobile`/`team_id`-gated → **s18 stays `3ac88d40…`** (re-verified each). Native at last gate **692/692**.

- **Inner-origin fix (the mobile reverse-ack root cause).** `enqueue_data` packed the DM inner with `_node_id`, diverging from `stamp_origin`'s `item.origin` (= `home_id` for a registered mobile) which rode the RTS. So a mobile's inner `origin` was its static-invisible local id → the recipient E2E-acked an unroutable id → RREQ storm. **Fix:** pack `item.origin` in `pack_unicast_inner` **and** `e2e_seal_inner` (`node_mac.cpp` grep `item.origin`). For a static node `item.origin == _node_id` → byte-identical. **Sanctioned crossing (invariant D):** a mobile's DM `origin` = `home_id` (the routable proxy); the true sender is `source_hash`.
- **Reverse-ack to a mobile originator (Option A).** The `E2E_ACK` echoes `SOURCE_HASH` = the mobile's hash when the acked DM's sender ≠ its origin (`key_hash_of_id(origin) != sender_hash`, `node_mac.cpp` grep `oh != sender_hash`); the home recognizes it (`node_mac_rx.cpp` grep `mobile_reverse_ack`) and last-miles it. Delegated sends translate `ctr_H→ctr_M` via a home-local map (`_deleg_acks`, no wire). **Crossing = the home last-mile** (already sanctioned). A static→static ack never echoes (gate is `known-and-different`) → byte-identical.
- **Team route-convergence.** A team-member mobile now DOES fire triggered beacons (`schedule_triggered_beacon` guard narrowed to `is_mobile && team_id==0`), and a received team RTS reverse-learns the sender into `_rt_team` **AND the `_team_peer` bitmap** (`node_mac_rx.cpp` grep `team reverse-learn`; **⚠ QA note: `is_team_peer` reads the BITMAP (`node_routing.cpp`), not `_rt_team` — both must be set or route-selection mis-routes**). Team-plane only; the beacon is `is_mobile` so the static plane drops it from `_rt` (`node_beacon.cpp:687`).
- **Cross-leaf team RTS.** A team RTS to our `_team_local_id` (`addr_len=1`) is exempt from the `handle_rts` leaf gate (`node_mac_rx.cpp` grep `team_rts_for_us`) — the team plane is leaf-agnostic (a mixed-registration team spans leaves). Gated on `team_id!=0` → static byte-identical.
- **Team key cache (E2E by team_local_id) — a NEW team-scoped store.** `LayerRuntime._team_keys[16]` caches a same-team beacon's `key_hash32` (`team_key_set` at `node_beacon.cpp` grep `team_key_set`), **NEVER `_id_bind`** (invariant A/§18 — a static receiver's `same_team_beacon` is false, so it caches nothing). `enqueue_data` DST_HASH falls back to `team_key_of_id` (gated `team_id!=0 && is_team_peer(dst)`). Pubkey via the team-scoped `WANT_PUBKEY`. **New QA item:** confirm `_team_keys` is populated ONLY on a same-team beacon and read ONLY for a team-peer dst (never leaks a key into the static id-plane).
- **Home-liveness + fail-loud guard (behavioral, no wire).** `last_heard_home_ms` refreshed by any home CTS/RTS (not only its beacon) + threshold `max(90s, 2×beacon_period)`; a reply-expecting send with no routable home fails loud (`mobile_no_home`) instead of storming.

**Canonical addendum spec for this arc:** `docs/superpowers/specs/2026-07-11-mobile-reverse-ack-to-originator.md`. **Limits:** encrypted team size ≤16 concurrent peers (`_team_keys` + `cap_peer_keys` both 16, LRU); plaintext team ~238.
