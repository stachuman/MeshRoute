...
| Mesh/Mobile | **mobile nodes + teams** — fw spine + the S1–S6 JSON surface | ✅ fw | **D30 — landed 2026-07-16** (gated); app adoption ↓ |
| Messaging | **⚠ BREAKING: `0x`-prefixed hashes + the `-t` team plane** (`send`/`resolve`/`reqpubkey`/`lookup`) | ✅ app | **D30 (a) — done 2026-07-16** (`SendDM.teamPlane` · `reqPubkey(_,team:)` · every hash arg 0x); lockstep with the reflash |
| Mesh/Mobile | decode S1–S6 — ready/cfg mobile+team · `mobile_reg`/`team_reg` · `mobile_status`/gw stream · `team_id` channels · peer names | ✅ app | **D30 (b) — done 2026-07-16** (89 tests; `MessageEntity.teamID` · auto-label via `NodeEntity.peerName`, never auto-contact) |
| Mesh/Mobile | UI: mobile connectivity chip + team chip (Device) · team badge on bubbles | ✅ app | **D30 (c1) — done 2026-07-16** |
| Mesh/Mobile | UI: **team chat threading** — `ThreadKey.teamChannel`; team posts = their own conversation | ✅ app | **D30 (c2·1) — done 2026-07-16** (90 tests; mock demo button) |
| Mesh/Mobile | UI: roam screen + three-plane contacts (Add-teammate · auto `-t` dispatch) | ✅ app | **D30 (c2·2) — done 2026-07-16**; D30 app adoption COMPLETE (91 tests) — bench-verify with the reflash |
| Mesh/Mobile | **Team surface** (D31/P3 map-less) — status card (homed/off-grid badge · pinned team chat · add-teammate) + a default **Team** filter + team-post last-seen | ✅ app | **done 2026-07-16** — the map drops onto it when fw ask 2 lands |
| Teams v1 | **Team QR (T-K4)** — `TeamCard` payload + Share (gated, private-key warning) / Scan → provision · grant-key action · key-state + no-key prompt | ✅ app | **D32 built 2026-07-26 → D33 RECONCILED to as-built 2026-07-31** (`team exportkey` · `team_ch_key` · `team <0xid>` grammar); key never persisted |
| Teams v1 | **`-l` per-send location** — compose toggle · bubble marker · `no_location` vs `unsealable` refusals | ✅ app | **D33 — done 2026-07-31** (shipped firmware; `loc_dm` deleted, never adopted) |
| Teams v1 | **CL2 channel encryption (app interface)** — `-t`/`-e` matrix · seal-state banner · location-in-team-posts (default ON) | ✅ app | **Track C done 2026-07-31** (103 tests); `-l` on channels stays un-emitted until T-K5 defines it |
| Teams v1 | **`conf`-gated encrypted send** (`overheard\|authoritative\|pinned`) | ⏸ | **Track D** — address-book spec unbuilt; fixes offering a doomed encrypted send |
| Platform | **Settings screen** (§7.6 — the oldest missing piece): notifications · units · display · security mirror · about | ✅ app | **done 2026-07-26** (Device → Settings; phone prefs only, node config stays on Device) |
| A Messaging | **Messages screen rework** — pinned team thread · compose with the missing **DM path** · team shortcut button · DM-first unread weighting · per-thread mute | ✅ app | **D34 — done 2026-07-31**; fixes "you could not start a DM from Messages" |
| B Contacts | **Verify / safety numbers + block** (§7.4) — verified state · full-key comparison · re-pin · block | ✅ app | **D35 — done 2026-07-31**; also fixes: the scanned pubkey was being **discarded** |
...
| D37 | 2026-08-16 | **Breakage sweep against the 08-16 firmware (3 found, 3 fixed).** **(1) `reqpubkey <bare id>` is AUTO, not team** (`console_parse.cpp:229-256`; `-t`/`-s` mutually exclusive) — the Add-teammate bootstrap would have hit `err_ambiguous_plane` or silently queried the STATIC plane. *Already corrected in-tree when I checked* (`Command.swift` emits `reqpubkey <id> -t`); verified rather than re-fixed. **(2) `team_channel_no_key` decoded the wrong field** — as-built is `{origin, layer_id, channel_id, channel_msg_id[, team_id]}` with **no body and no seq**; we read `team`, so the team was always nil. Fixed + the other fields captured. This one matters because **default-on sealing means a keyless member sees teammates "go silent"** unless this push is surfaced. **(3) `mobile_registered` changed MEANING** — it is now `mobile_attached()` (CONFIRMED), so a mobile mid-CLAIM reports `registered:false` + `attachment:"claiming"`; the schema didn't move, the semantics did. Reworked `MobileState` onto the §MH-S4 three-plane block (`attachment` · `home_link` · `home_confirm_age_ms`, plus claim/candidate detail) and **honoured the standing ruling that the word "connected" is FORBIDDEN**: the UI now renders the state machine plus **evidence** ("Home confirmed 7 min ago") instead of a green light, with the age **omitted, never 0**, when nothing was ever confirmed (a 0 age reads as "just now" — the exact lie that plane exists to prevent). Also added the five new ack codes (`err_ambiguous_plane` · `err_no_identity` · `err_tx_queue_full` · `err_resolve_pending_full` · `err_ack_ring_full`). 106 tests, app builds. |
| D36 | 2026-08-16 | **Companion reshapes into TWO CONTEXTS on one phone (owner direction) + the configurator firmware ask.** Owner: the companion's centre of gravity is the **mobile node**; **static + gateway are limited to configuration and diagnostics**. ⇒ **(1) mobile companion** — messaging/team/position, and since a mobile may be **screenless**, it must be complete enough to bring a node up from factory (the phone is then the ONLY UI); **(2) configurator** — *target-scoped* ("configure node **X**"), entered two ways: **A/direct** — BLE to a fresh static/gateway (✅ works today) and **B/indirect** — mobile companion → *view network* → pick a node → configure. One app, adaptive by attached-node profile; the target node is **named in the header at all times** (the hazard is ambiguity about which node you are changing, not complexity). **Entry defaults:** unprovisioned/not-joined ⇒ configurator (the "newly set up node" case); gateway ⇒ configurator (team/mobile verbs are **not compiled** there); mobile ⇒ companion; provisioned static ⇒ companion with configurator one tap away (a static has the full messaging surface too). ★ **Survey finding that reshaped the plan (verified in source, 2026-08-16): Path B is structurally impossible today.** `MR_FEAT_REMOTE_MGMT` is **one flag for both directions** and is 0 on the mobile profile — a mobile cannot be administered (correct, and the app wants to keep that) **but can still ISSUE `rcmd` with the sealing branch compiled out**, so a gated verb **flies cleartext, the target silently drops it, and the console prints a success-looking line** (`firmware_remote.cpp:182,192-210`). Also: the `rcmd` **response still hardcodes the USB console** (`fw_main.cpp:1464-1474`) though the BLE sink exists (`:311-313`), and **challenge–response was never built** — the source calls its own scheme *known-broken by design, redesign owed* (`firmware_remote.cpp:72-90`). ⇒ Wrote **Ask 3** into `INBOX_SYNC_CONTRACT.md` (living, amendable by both sides): split **ORIGINATE vs ACCEPT** (mobile: originate 1 / accept 0 — the flow *starts* on a mobile), route the `rcmd` response through the existing sink **as JSON**, build challenge–response, and give the app a **renderable failure vocabulary** (cannot-be-administered · no-admin-key · locked · stale · no-route) because "silently dropped" is unusable in a UI. **App meanwhile:** build the configurator target-scoped with capability gating so remote is a *transport swap*, not a redesign — and never surface a mobile-originated `rcmd` as "sent" while the foot-gun stands. |
| D35 | 2026-07-31 | **Verify / safety numbers + block (§7.4).** ★ **Found a real gap first:** a scanned card's **full pubkey was being discarded** — passed to `peerkey` and dropped — and `NodeEntity.verified` was **never set by anything**, so the trust tier the model carried was always false. Both fixed: `NodeEntity` gains **`pubkeyHex`** (the peer's full `ed_pub`, opaque bytes per D6) and **`blocked`**; a QR scan now PINs on the node **and** records the key + verified locally. **Safety numbers, honestly:** the app is crypto-free, so it does **not** invent a fingerprint (a hash we derive app-side would have no firmware counterpart to compare against). Instead node-detail shows **the real key material** — the 64-hex `ed_pub` in 4×4 groups — for the two people to read aloud; that IS the identity, whereas `key_hash32` is 32-bit and **grindable**, which the UI now says out loud on an unverified contact. Unverified contacts get **"Verify by scanning their card"** (the ceremony → PINNED, which an on-air answer can never overwrite); verified ones get **re-pin** (for after a node reflash/reprovision, where the app holds the key but the node may have lost it). **Block** is app-side and states its own limit: messages are still received and archived (the radio can't be muted) but the thread is hidden from Messages and never notifies. Phone-local like mute/`isRead`. 103 tests, app builds. |
| D34 | 2026-07-31 | **Messages screen rework — pinned team + one compose (user-proposed two buttons; refined in discussion).** Three faults found: (1) ★ **you could not start a DM from Messages at all** — the only compose button was *New channel post*, so a conversation had to be started from Contacts/Mesh; (2) the team conversation drifted down the chronological list even though it is the primary thread for P3; (3) *New channel post* demanded a channel number, clumsy for a team with one conversation. **Ruling — NOT two "new" buttons:** the two cases are asymmetric. A DM genuinely needs *creating* (pick a recipient) → belongs behind a compose button; the **team conversation already exists**, so a "New team message" button would open a compose sheet for a thread already in the list — one tap *slower* than simply keeping that thread visible. **Built:** the **team thread is PINNED** in its own top section (teal `person.3`, unread count, synthesised when empty so the row is always tappable) — 1 tap, never buried, and absent on a static node (D31 adaptive); **one compose button → `NewMessageSheet`** with two paths — **Direct message** (a recipient picker over contacts + teammates → straight into the thread) and **Channel/Team post** (blank channel = the team's default, so a hiker types nothing but the message; footer states whether it will go out encrypted). `NewChannelSheet` retired. 103 tests, app builds. **★ Owner rulings, all three built the same day:** **(a) the flip is IN** — a fixed-position team button (`person.3`) now sits beside the compose button and **opens** the thread (never a compose sheet), so team chat is reachable by muscle memory under gloves/stress *and* the pinned row still carries its unread + last-message context. **(b) DMs always outrank broadcast traffic — weighted VISUALLY:** an unread **DM** gets the full-weight treatment (leading accent dot · bold title · white-on-accent count), while unread **channel/team** stays deliberately quiet (grey count, no dot) — a chatty team can never drown out a person messaging you. **(c) per-thread mute** (phone-local, like `isRead`, never sent to the node — keyed by a new stable `ThreadKey.storageKey`): long-press any thread → Mute/Unmute; a muted thread keeps its in-list count but **never badges and never notifies** (guard added in `notifyInboundDM`; the app-icon + tab badge now count only unmuted unread). |
| D33 | 2026-07-31 | **Firmware re-sync: team key is LIVE, location moved to `-l` — Tracks A+B built.** Reviewed the 07-16→07-31 wave (contract 596→830 lines, new specs `2026-07-30-channel-crypt-and-location-privacy`, `2026-07-29-peer-address-book`, the open-bug register). **Track A — reconcile the team-key surfaces to as-built:** ⚠ my D32 guess was **wrong** — the keypair is NOT in `cfg`; it comes from a dedicated **`team exportkey`** → `team_key_export{team_id,tkpub,tkpriv}`, with refusals as **distinct events** `team_key_err{no_key\|no_team}` (never a null-bearing success object). Fixed: the export path; **lock state now reads `team_ch_key`** (in `ready`, omitted when teamless; always in `cfg`) instead of being inferred; full `team_key_received{team,hash,origin,name?}` (the key is **already adopted** when it fires); `team_key_grant{hash,ctr,parked}` decode. **Provisioning grammar corrected** — as-built is `team <0xid> [freq=<MHz> sf=<5-12> bw=<kHz>] [tkpub= tkpriv=]`: the id **must** carry `0x` (2026-07-30 grammar — a bare `88A672BA` used to join *team 88*), and the verb **rejects unknown keys**, so my `sf_list=`/`cr=` would have failed loud; the QR's data-plane params now go via **`cfg set sf_list` / `cfg set cr`** (owner note: `cfg set control_sf`/`sf_list` remain the direct setters). ★ **Key-handling ruling:** the contract's accepted risk (`exportkey` works on **any transport incl. BLE, no auth gate**) obliges Keychain-not-plist; we go **stronger — the pair is never persisted at all**: fetched on demand, held in memory only while the Share sheet is open, purged on dismiss, and **never logged** (the console line prints the team id only). **Track B — the `-l` location slice** (shipped firmware; `cfg set loc_dm` + TLV `0x18` are **deleted** — the app never adopted them ✅): per-message location toggle in compose (DM-only; `send_channel` has no `-l`, `send_layer -l` refuses), a location marker on the bubble, and the two refusals surfaced **distinctly** — **`no_location`** ⇒ "no fix — set the location or wait for GPS" (★ explicitly **not** a key prompt) and **`unsealable`** ⇒ "can't send location unencrypted" + an offered Request-key/QR, **never auto-issued, never silently retried**. 102 tests, app builds. **Still blocked:** CL2 team-channel encryption is unbuilt (zero hits in the tree) — and it gates AB4 (stored peer location), being the trust anchor. **Track C — the CL2 interface, DONE 2026-07-31 (ahead of the firmware slice):** `SendChannelPost` gained `teamPlane`/`encrypt`/`attachLocation`, and the encoder makes the two **REFUSING** combinations structurally unemittable — `-e` without `-t` is dropped (no key exists for a global channel) and the `-t -g -e` trap is unreachable because the app never offers `-g`. A team thread now posts `-t` explicitly (+ `-e` when the node holds the key, mirroring `team_channel_crypt` default-ON); an un-keyed member still posts **plaintext** (keyholders read it) rather than being blocked. ★ **Owner ruling: location in team posts defaults to YES** — a Settings toggle (`shareLocationInTeamPosts`, default ON) plus a compose banner on team threads showing exactly what will go out ("Encrypted to your team · + location" / "Plaintext — no team key"). The interaction that makes this safe: a channel location is `inner_type = 1` and **requires the crypted flavour** (§2.4), so the preference is **automatically inert whenever the post can't be sealed** — no un-keyed member ever leaks a position, and the toggle needs no caveat from the user. ⚠ **`-l` is NOT emitted on `send_channel` yet** — its console form is undefined (today `bad_args`); the encoder + tests are ready and one flag in `dispatch` flips when T-K5 lands. **Next:** Track D (`conf`-gated encryption from the address-book spec: gate "send encrypted" on `conf >= authoritative`, not key presence — today we can offer a send that then fails `no_pubkey`). |
| D32 | 2026-07-26 | **v1 teams arc + Settings — companion built AHEAD of the firmware slices** (specs `2026-07-26-team-encrypted-channel-design.md` T-K1–T-K5 + `2026-07-26-companion-v1-feature-roadmap.md` §A; the companion is a **v1 release feature**, not a fast-follow). **Team QR (T-K4 — a companion-owned slice, so built for real):** a `TeamCard` payload per §2.4 (`ver·team_id·freq_khz·bw_hz·routing_sf·sf_list bitmap·cr·team_ch_pub·team_ch_priv·name` + **CRC32**, little-endian packed → base64url). ★ **Deliberate security call: the team card is NOT an https URL** (unlike `ContactCard`) — it carries a PRIVATE key, and an https QR opened by a stock camera would ship the whole query to meshroute.eu + browser history; it uses `meshroute://team?d=…`, which can never resolve to a web request. UI: a **Share team** screen gated behind an explicit *Reveal* with a red private-key/no-revocation warning; a **Scan team QR** flow (review PHY + team, then one-tap provision); **Grant team key** on a teammate's detail (T-K3); **key-state** (`teamHasKey`) + the `team_channel_no_key` → "ask a teammate for the key" prompt on the team card. Decode added for the two ratified-by-name pushes (`team_key_received`/`team_channel_no_key`, tolerant shapes). ⚠ **Pending firmware confirmation** (inert if wrong, one-line fixes): the provisioning verb tokens (`team key= freq= bw= sf= sf_list= cr= tkpub= tkpriv=`, read from spec §1+§2.4) and the **cfg round-trip field names** for the keypair (`tkpub`/`tkpriv` — absent ⇒ the share screen shows its empty state; **the app never invents key material**). **Settings screen** (roadmap §7.6, the oldest missing gap): notifications (DM/team/badge) · display (units, `#ctr` counters) · a read-only **security mirror** (encrypt-default, team-key state) · about — under Device, with a strict rule stated in-file: **phone prefs live here, node config stays on Device**. 99 tests, app builds. |
| D31 | 2026-07-16 | **User profiles — the three usage scenarios now drive the UI (user-proposed; supersedes the flat one-size shell).** **P1 static-network operator** — configure/maintain, (i) *direct* over BLE (today's Device tab) and (ii) *remote* through another node (the authenticated-`rcmd` path — UI to build, firmware adjustments expected); occasional purposeful sessions. **P2 lone mobile** — receiver/sender registered to a home, messaging the wider network; continuous background use, notifications+battery over config; the features exist, the *prominence* is wrong (a mobile must not see leaf join/create — it registers to a home). **P3 team / hike mode** — team chat + teammates always; *plus the rest of the network iff homed* (the plane split delivers exactly this duality); **map-less** (members·last-seen·pinned team chat·off-grid/homed badge — buildable now) vs **map** (needs position distribution — firmware). **Decisions:** (1) **adaptive UI, not a mode picker** — the connected node's `ready` (static/gateway·mobile·team·homed) drives the *defaults*; profiles materialize as *onboarding flows*; everything stays reachable via progressive disclosure; revisit if adaptivity confuses. (2) **remote admin greenlit** — the fw basis (authenticated `rcmd`) exists; expect firmware adjustments to fit the app (JSON-consumable responses, unlock/lock flow) → ask §8.2. (3) **hike-mode positions — DECIDED (user-confirmed): TEAM-PLANE ONLY, fully separated, visible in-team only**, with a two-tier position store: **RAM = the live phone-fed position** (volatile `pos` verb, never persisted — no flash wear; lost on reboot, the phone re-feeds) and **NV `/mrid` lat/lon = a user-set DEFAULT** (changed only by a dedicated `cfg set lat/lon` action, never auto-updated by the feed; effective = RAM-fresh else default). The **BCN ext-TLV mesh-wide broadcast stays the separate Theme-C fleet-map rail** (operator visibility, opt-in, later); sparse-team caveat accepted (member-to-member propagation only) → ask §8.2. First adaptive slice shipped same day: a mobile's Device tab hides leaf membership/join/create (it registers to a home — the chip/roam lead instead). |
| D30 | 2026-07-16 | **The mobile/team wave — three-plane addressing + the S1–S6 JSON surface (contract/handover 2026-07-13…16).** The network model is now complete: **static · gateway · mobile**. Firmware landed (routing spine committed; the JSON surface gated/uncommitted): team 6.2/6.3/6.4 + multi-hop routing parity + plane separation; the **HARD PLANE SPLIT** — `send`/`reqpubkey` gained **`-t`** (TEAM plane): a teammate is reachable ONLY with `-t` (`send <team_local_id> "…" -t` / `send <0xhash> "…" -t`; encrypted = `reqpubkey <0xhash> -t` then `send <0xhash> -t -e`); a home-attached NON-teammate mobile stays plain `send <0xhash>` (firmware resolves via its home — the one no-app-change case); a plain send to a teammate `no_route`s. **⚠ BREAKING (2026-07-13): every key_hash32 command arg MUST be `0x`-prefixed** (`send`/`send_layer`/`resolve`/`reqpubkey`/`lookup`) — bare-8-hex autodetect is GONE (`bad_args`); bare decimal = always an id (static node_id, or team_local_id under `-t` — a **distinct id space**, never mixed with static ids in the contact model). **S1–S6 JSON (build against NOW):** `ready` +`mobile`/`mobile_registered`/`mobile_home`/`mobile_local`/`mobile_home_layer`/`hosting`/`team`/`team_local` (ALL omit-when-inactive; team ids = **quoted hex strings** like `key`); `cfg` +`mobile_autoregister`+`team_id` (always present, `"00000000"`=unset); `mobile_reg`/`team_reg` pushes (register/roam/home-loss + team-DAD); `mobile status`→`mobile_status` JSON (int `freq_khz`/`bw_hz`), `mobile gateways`→ streamed `mobile_gw`*/`mobile_net`*/`mobile_gw_end` (roam-UI data), `mobile_err{not_mobile}`; **`team_id` (hex string) on `channel_recv`+`inbox_channel`** (omit ⇒ leaf channel; `channel_msg_id` stays the dedup key; ⚠ the store-version bump wipes the on-node inbox on first boot + bumps `inbox_epoch` — the app's epoch re-pull handles it); **peer names**: `peer_key_cached` +`"name"` (omit-when-unknown) + `nameof 0x<hash>` → `peer_name{hash,name?}` → **contacts auto-label** (app rule: the self-reported name lands in a NEW `NodeEntity.peerName`, NOT the user-given `name` — so auto-labeling does not auto-promote a heard node to a contact; display = name ?? peerName ?? id ?? hash). Also: `send_channel` on a team mobile auto-broadcasts to the team (REJECTS `-t`); `rcmd` is now **authenticated** (sealed except `status`/`routes` — fire-and-observe); `send_failed.reason` +`mobile_no_home`; expect un-rendered mobile↔home keepalive traffic (not app-visible). **App plan → (a)+(b)+(c1) DONE 2026-07-16** (same session): (a) `Command.swift` — every hash arg `0x`-prefixed (`send`/`resolve`/`reqpubkey`/`lookup`), `SendDM.teamPlane` → `-t` (flag order `-a -e -t`), `reqPubkey(_, team:)`; (b) full S1–S6 decode — `NodeReady` +8 mobile/team fields, `NodeConfigInfo` +`mobile_autoregister`/`team_id`, new `mobileReg`/`teamReg`/`mobileStatus`/`mobileGateway`/`mobileNet`/`mobileGatewaysEnd`/`mobileError`/`peerName` events, `channel_recv`+`inbox_channel`+`InboxEntry`+`MessageEntity` gain `teamID`, `peer_key_cached` gains `name` → **auto-label into `NodeEntity.peerName`** (display = name ?? peerName ?? id ?? hash; never auto-promotes to a contact); AppModel observable `mobileState`/`teamID`/`teamLocal`/`latestMobileStatus`; mock serves reqpubkey-name + `nameof`; **89 wire tests, app builds**; (c1) Device→Network **mobile connectivity chip** ("Registered · home N · L4" / "Searching for home…") + **team chip** (id + our `team_local`) + a team badge on channel bubbles. **(c2·1) Team chat threading DONE 2026-07-16:** `ThreadKey` gained **`.teamChannel(team:channel:)`** — a team post is its OWN conversation, split from the same-numbered leaf channel end-to-end: ingest (live + pull) builds the team thread key; `MessageEntity` stamps/round-trips `teamID`; the leaf-channel predicates now EXCLUDE team rows (fetch/mark-read/ThreadView); an outgoing `send_channel` on a team node threads under the team conversation (the firmware team-scopes it — same verb); Messages rows show a teal `person.3` icon + "Team · Channel N" (+ `team <hex> · ch N` subtitle); dedup identity unchanged (`channel_msg_id`); ConversationStore mirrors the split (tested: same channel number → two threads, live + pulled). Mock: `simulateChannel(teamID:)` + durable team tag + a "Simulate team channel post" demo button. 90 tests, app builds. **(c2·2) Three-plane contacts + roam screen DONE 2026-07-16 — D30 app adoption COMPLETE.** *Contacts:* `NodeEntity` gained `teamID`/`teamLocalID`; **the hash stays the universal DM identity** — the `-t` plane is **derived at dispatch** (`isTeammate`: the recipient's `teamID == ours` ⇒ `send 0x<hash> … -t`; pseudo-id threads stay static; no compose toggle). **Teammate bootstrap** = the contract's bare-id team request: an *Add teammate* sheet (Mesh toolbar, team members only) fires `reqpubkey <team_local_id>` → the mutual handshake's `peer_key_cached{hash,name}` auto-creates + stamps the directory row (pending-id correlation; role=mobile). Node detail gains a *Request key (team)* action + a Team membership section; teammate rows show `person.3` + "team id N"; `send_failed{mobile_no_home}` → an orange "register first" bubble. *Roam:* `MobileRoamView` (Device → the connectivity chip) — registration + PHY + auto-register state, *Re-register* / *Scan networks*, and the learned-networks list (`mobile_gw`/`mobile_net`/`mobile_gw_end` accumulated on the routes pattern) with tap-to-register (`mobile register freq= sf= bw=`, fractional-safe). New verbs: `reqPubkeyTeam(localID)` (bare = team), `mobileStatus`/`mobileGateways`/`mobileRegister`/`…Scan`/`…Target`. Mock now plays a **registered team mobile** (ready mobile+team fields, `mobile status`/`gateways` demo data, bare-id handshake with synthetic teammate hashes) so the whole surface demos in the sim. 91 tests, app builds. **T1000-E note:** a screenless mobile-only tracker variant is feasibility-PARKED — if built, the companion is its ONLY management UI ⇒ design the mobile screens phone-first. Refs: INBOX_SYNC §*Mobile node + teams* + ★Commands, HANDOVER ★2026-07-16, `2026-07-16-companion-mobile-team-json-surface.md`. |
...
| ⭐ ❌ **Remote-admin app surface** (D31/P1): app-consumable (JSON) `rcmd` responses + an `unlock`/`lock` flow over BLE with acks + an error model (no_admin_key/locked/stale) — the authenticated spine EXISTS, the app-facing shape doesn't | M | **remote node configuration** (§7.1) — scenario P1-remote; detailed ask in INBOX_SYNC §*Firmware asks — profiles* |
| ⭐ ❌ **Team position sharing** (D31/P3 hike mode): a **volatile** phone-fed position verb (RAM — `cfg set lat/lon` NV-persists ⇒ flash wear) + **team-plane** distribution (cadence/on-move) + a member-position push to the app | M | the **team map / bearing list**; my-position feed; detailed ask in INBOX_SYNC §*Firmware asks — profiles* (BCN ext-TLV stays the separate Theme-C fleet rail) |
