...
- **Phase 2 (app-only): Map v1 + node detail + Settings.** Fixed-node map + bearing fallback · node-detail screen (verify/block/rename/rcmd) · Settings section.
  **Re-scoped by D31 (user profiles, 2026-07-16):** Phase 2 is now profile-driven — **(i)** adaptive defaults by connected-node role (first slice shipped: a mobile's Device hides leaf provisioning); **(ii)** ✅ the **Team surface** (P3 map-less) — **done 2026-07-16**: a Mesh-tab team status card (team id + your overlay id · a **homed "Team + network" / off-grid "Team only"** badge · a pinned **Team chat** button → the most-recent team thread, else the app-default team channel 1 · Add teammate) + a **Team** segment that is the *default filter* on a team node (adaptive, session-once; falls back if the team vanishes) + teammates' `lastSeen` refreshed by their team posts (matched by team-local id, never static ids); **(iii)** **remote-admin UI** (P1-remote) + **team position sharing → the team map** (P3) once the two D31 firmware asks land (INBOX_SYNC §*Firmware asks — profiles*); **(iv)** per-profile onboarding flows; Settings + verify/block as before. The mesh-wide BCN position broadcast stays Phase 3 (the fleet map).
