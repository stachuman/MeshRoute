...
> **⚠ AS-BUILT 2026-06-22 — code DONE + green (uncommitted); behavioral gate UNTESTED, TO VERIFY (dedicated tests skipped for now, user call).** PASSED: native **490/490** (new mobile unit) · BASELINE suite **no-regress + inert** (s18 98/113 · s16 57/80 · s19 8/8 *exact* · leaks 0 — no mobile nodes in the suite ⇒ the guard never fires) · **3 boards SUCCESS** · implementation verified faithful. **NOT proven end-to-end:** the exclusion is **narrowly-live** — a route-through-mobile arises *only* via F-discovery (QG-validated: mobiles beacon identity-only `node_beacon.cpp:310` AND still relay RREQs — **no `is_mobile` guard in the forward path** — so it is never beacon-learned, only RREP reverse-path). So the `t1xx` gate below must use an **F-discovery-through-mobile** topology (buildable, not built); `next_hop_selectable` (pick-time) + the sole-transit graceful-defer (the Phase-1 starvation lesson) are also unit-uncovered (only `rt_merge` learn-time is). **TO VERIFY when tests resume:** that F-discovery scenario *or* a `route_inject`-forced route-through-mobile native test.
...
> **✓ AS-BUILT 2026-06-22 — DONE + GATED-PASS (uncommitted).** native **491/491** · BASELINE **no-regress** (s18 98/113 · s16 57/80 · s19 8/8 · leaks 0) · **3 boards SUCCESS** · payload-len match **QG-verified** (`node_mac.cpp:558` builds every RTS `payload_len = inner_len + data_mac_len(flags)`; inner+MAC is the E2E-sealed body, CRYPTED flag end-to-end → forwarded unchanged → a flag mismatch can only *miss the benefit*, never false-cancel — the spec's stop-and-ask point resolved soundly, no ask needed). **LIVE behavioral evidence (stronger than a dedicated sim):** `implicit_ack_from_forward` fires **20× on s18 with delivery held exactly at 98/113 ⇒ zero false-cancels** (a single false-cancel would drop `arrived` below 98). s16/s19 don't exercise it (0 fires, no lost-ACK multi-hop) and hold baseline. The `t1xx_implicit_ack_forward` sim is **deferred** — the suite already proves fire+safety; only the per-event airtime-win *quantification* is unmeasured (less debt than ①). ⚠ **Companion follow-up (coder-flagged, NOT built):** no `send_acked` Push on implicit-ACK (faithful to Lua) → an implicit-ACKed send shows **un-acked in the iOS app** though the hop succeeded; verify `send_acked` is hop- vs e2e-level before adding.
