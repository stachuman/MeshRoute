...
- **OPEN DECISION — default PIN (logged 2026-06-12; revisit before fielding any `ble_mode != off` node):** the passkey defaults to `g_ble_pin` = **`123456`** (shared), which defeats the MITM auth — anyone who knows the default just pairs. **Recorded resolution (NOT yet implemented):** change the default to a **per-device random** 6-digit minted on first boot (HW RNG, SD-safe via `device_rng`'s `sd_enabled()` branch) + printed once over USB at boot; `cfg set ble_pin` still overrides. Do **not** derive it from `key_hash32` (broadcast in beacons → guessable). `123456` is fine for the closed bench, **not** for the field.
...
8. Scope boundary: Phase 2 serves ONLY the live push ring (msg_recv/channel_recv/etc.). pull_inbox/mark_read and the sender_hash/channel_msg_id **+ `seq`** push-field additions (INBOX_SYNC_CONTRACT; `seq` = the live+pull unified-cursor **"model B"**, decided 2026-06-12 — a connected client gap-detects a dropped best-effort push and delta-pulls) are Phase 3 — confirm they stay OUT of this scope even though the inbox stores are already stubbed in fw_main.cpp.
