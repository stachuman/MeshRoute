...
**Step 4 — `device_rng` SD-aware branch.** [BENCH-only to exercise, NATIVE-compilable]
- `src/device_rng.h` (caveat at :11-12): when the SoftDevice is enabled, the direct `NRF_RNG->VALUE` reads (`:33-41`) are illegal. Add a branch inside `mrrng::fill`: if `sd_softdevice_is_enabled()`, call `sd_rand_application_vector_get()`; else the current path.
...
**Step 5 — BLE transport backend.** [BENCH-only]
- New device file (`src/device_ble.h` / wired into `fw_main.cpp`): Bluefruit `BLEUart` (Nordic UART Service). `Bluefruit.begin(1,0)` at boot when `ble_mode != off`; `bleuart.begin()`; RXD callback → `feed_line()` (the §1 messaging refactor) + the `whoami`→`write_ready` responder; TXD ← chunked `write_push`/`write_ack`/`write_ready`.
- Main-loop integration (`fw_main.cpp:603-688`): call `g_companion_policy.on_tick(now)` → `Bluefruit.Advertising.start/stop()` per the window. **Fan the single push drain (:635) out to BLE + USB — do NOT add a second `next_push` loop** (§1: `next_push` is a destructive pop).
- Bluefruit52Lib is **present in the BSP** (`framework-arduinoadafruitnrf52 @ 1.10700.0`, confirmed `~/.platformio/.../Bluefruit52Lib/src/`); no new `lib_deps`.

**Step 6 — Security / pairing.** [BENCH-only]
- `Bluefruit.Security.begin()` + `setPIN(cfg.ble_pin)` + `setMITM(true)` + `setSecuredCallback()` gating GATT to an encrypted+bonded link before serving characteristics (spec §A.3 — mandatory, not later). Bonding auto-persists to InternalFS.
...
**Step 8 — Test client.** [Dev-tool, not unit-tested]
- `tools/meshroute_client_ble.py` (bleak): scan for the node name, connect, pair with PIN, NUS UUIDs (`6e400001/2/3-...`), write line-ASCII, buffer notifies to `\n`, print JSON. Doubles as the iOS transport reference. NB: the repo has **no `pyproject.toml` at all** (the `tools/` are loose scripts) — introduce a `bleak` dependency via whatever convention you add (create `pyproject.toml` or `requirements.txt`); `bleak` is not currently vendored.
