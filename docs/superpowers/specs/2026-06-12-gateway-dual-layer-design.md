...
3. **DAD per layer:** **two independent `join` runs (one per leaf) at boot**, both bound to the one `key_hash32`; **`whoami` surfaces both per-layer `node_id`s**. (DEFERRED to the join workstream — static `node_id` from cfg/NV for now; see [[project_meshroute_join_deferred]].)

**Resolved 2026-06-13 (Slice-4 build — cross-layer DM decomposition + decisions; full plan in `docs/plans/2026-06-13-slice4-crosslayer-dm.md`):** Slices 0–3e DONE (config/validation, LayerRuntime, activate_layer + window scheduler, per-leaf beacon, the gateway advertises its schedule + neighbours learn it + `gateway_schedule_defer_ms`). Slice 4 (the cross-layer DM activation) was workflow-decomposed + adversarially critiqued, then user-signed-off, into **`4a′→4g`** (each native-tested + 3-boards-green + gated): `4a′` Push/inbox `layer_id` field (spec §2/Q13 prereq — lands FIRST) · `4a` wire `gateway_schedule_defer_ms` into the send (the deferred 3e.2b) · `4b` layer-path inner codec (§5) · **`4c.1` KEYSTONE = the bridge fork** · `4c.2` `gw_relay`/`RTS_FLAG_RELAY` threading · `4d` `send_layer` origination (§5.1) · `4e` reversed-path E2E ack (§5/§0.10) · `4f` unknown-binding defer+H-flood+TTL · `4g` 2-node integration + s09/s10 parity (§8). Four decisions that REFINE the prose above:
- **(a) Bridge fork lives in the DELIVER branch** (`do_post_ack`, beside `l2c_handle_misdelivery`), gated `dst_key_hash32 != _key_hash32`, ordered AFTER the deliver-to-me / E2E-ack-for-me tests — CONFIRMS §5 Bridge step 1 ("G receives the CROSS_LAYER DATA *addressed to it*"). NOT the forward branch.
- **(b) Gateway-route recovery (user 2026-06-13): PARK + reactive `ROUTE_QUERY`**, refining §5.2. §5.2's "no gateway KNOWN → `err_no_gateway`, no fallback flood" stands for the genuinely-unknown case; the distinct **gateway-known-but-no-live-route** sub-case (the Lua Pass-2, dv:5207) parks the send + fires the standard reactive route discovery (explicit recovery, NOT a silent fallback; still `err_no_gateway` on TTL). A `ROUTE_QUERY` is route-recovery, not a DATA flood.
- **(c) Handoff buffer = cap 16, compact full-body (≤228 B) `XlHandoff` POD** (user 2026-06-13), Node-global beside `_gw_schedules`. The reserved `cap_gateway_deferred_handoffs=32` (§5.4) stays as the TTL/policy reference constant only, NOT the array size. A 64-B body cap was REJECTED (would silently refuse valid in-spec DMs — a correctness bug disguised as RAM).
- **(d) Giveup is LOUD ALWAYS** — the spec's §5.2 bridge-step-2 "(optional) `send_failed`" is tightened to a mandatory `send_failed` + telemetry on TTL drop (standing no-fallback rule).
- **Critic gap folded in (not in §5):** the cross-leaf re-inject must SEED the far leaf's `_seen_origins` for `(origin, new_dst, ctr)` at inject time, or G re-bridges its own relay forever (loop). Plus: a malformed `CROSS_LAYER` inner is REFUSED, never raw-fallback-delivered as body.

---
**Next:** user reviews §0–§9 (especially the open decisions); I fold the answers; then a coding agent implements against this + I quality-gate (native + the new sim layers scenario green; both boards build; metal = user bench). **STATUS 2026-06-13: Slices 0–3e DONE + signed off; Slice 4 decomposed + signed off; implementing `4a′`.**
