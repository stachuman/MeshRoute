...
chrome still have named metal acceptance. ⛔ **Phase A is NOT complete** — ⛔⛔ **BUT CORRECTED IN PLACE 2026-08-22,
AND THE WITHDRAWN WORDING IS KEPT VISIBLE: this read *"[[B164]] remains a metal truthfulness gate, while [[B189]]'s
recovery half remains an explicit post-diagnostic decision rather than an implemented retry"*. THE [[B164]] HALF IS
FALSE.** Bench **Part 22 WAS RUN ON METAL** in the 2026-08-20 session —
`docs/superpowers/plans/2026-08-20-metal-session-walkthrough.md` **PHASE 4c (`P 22.1-22.2`), headed *"confirmed
OK"***: `AIRED`/`ACKED` on a DM, the **full** channel handle, and the panel reaching `SENDING...` → `SENT, waiting`
→ `PICKED UP` / `NO RELAY HEARD` (owner, 2026-08-20: *"whole walkthrough done, no more remarks"*). ⚠ **The [[B189]]
half STANDS UNCHANGED and is deliberately not corrected away.** ★ **What actually keeps Phase A open — read it from
the live records named here, ⛔ never from a tally restated in this header:** the **§UI-17 full-UI metal session**
(`docs/superpowers/specs/2026-08-20-ui17-navigation-status-team-redesign-spec.md` §7, which by owner ruling also
ABSORBS the deferred [[B231]]/[[B233]] inbox recheck, bench 27.17 / [[B230]] and the [[B232]] press sequences) · the
metal residue each 🧪 row below names for itself (UI-7D's bench Part 19 + [[B134]]; UI-8's H8 matrix; UI-9's
control-line and radio-load arms; ⛔ **and [[B193]] — CORRECTED IN PLACE 2026-08-22, ONE DAY AFTER IT WAS WRITTEN INTO
THIS LIST: the withdrawn item read *"[[B193]]'s real-flash / power-cut half, still carried as owed in
`docs/2026-07-30-open-bug-register.md`"*.** Bench **Part 20.5 RAN AND PASSED** — run-sheet §9 `[x]`
(`docs/superpowers/plans/2026-08-18-metal-session-run-sheet.md`) **plus the explicit owner confirmation of 2026-08-22
that the mark means ran-and-passed**; **register [[B193]] is CLOSED** on that record, and the `/mrjoin` sibling was
qualified separately by the §UI-15 slice-7 power-cut. ⚠ **What survives of it is NOT a Phase-A hold:** flash **wear**
(Part 20.5 step 5 disclaims it) and the **nRF52/LittleFS** write path (the bench is ESP32/NVS) — both carried on the
UI-13/UI-14 rows) · and [[B189]]'s recovery decision. ⛔ **UI-10/UI-11/UI-12/UI-16 are
EXTENSION scope and are not Phase-A completion items.**
Sections marked 📝 are design only and must not be read as shipped behavior. The 2026-08-06 UI-13–UI-16 extension
adds staged settings, direct provisioning and nearby-team onboarding; ⛔ **CORRECTED IN PLACE 2026-08-22: this closed
*"UI-15/UI-16 remain design only"*, which is FALSE for UI-15** — it landed in full (slices 1-7) and is metal-qualified
by the same walkthrough (Phases 3, 5, 6 and the Phase-7 `/mrjoin` power-cut). **UI-16 remains design only.***
...
> **B123**'s pre-3.2 park residual. ⛔ **CORRECTED IN PLACE 2026-08-22, withdrawn not deleted: this closed *"The later
> owner-requested UI-13–UI-15 scope extension is separately marked 📝 and has its own open decisions in §14"*. UI-13,
> UI-14 and UI-15 have ALL LANDED** (§UI-13/§UI-14 2026-08-13; §UI-15 slices 1-7, metal-qualified by
> `docs/superpowers/plans/2026-08-20-metal-session-walkthrough.md`) — the 📝 extension that survives is **UI-16**, and
> §14's open questions are read there, not inferred from this banner.
...
| area | status — each cell carries its own correction date | evidence / remaining work |
|---|---|---|
...
| UI-8 emergency hardware qualification | 🧪 | all render/send arms exist; the H8 bench matrix is still the completion gate. ⚠ **QUALIFIED 2026-08-22 — PARTIAL, ⛔ not discharged:** the emergency **screens** were exercised on metal 2026-08-20 (walkthrough Phase 4b step 21 = bench **25.3**, the CHROME safety arm: no rail, strip present, every headline complete at the left edge, one alarm run to `NOT RELAYED` and one cancelled). ⛔ That is the *rendering* arm only — the **H8 nine-case matrix itself** (bench Part 8's 8.23-8.27 and guide `2026-08-04-heltec-v3-oled-ui-bench-guide.md` H8-01…H8-10: the first-post attempt counter, fire-from-dark, reply-wakes) is **not recorded run** and remains the completion gate |
| UI-9 V3 battery reader | 🧪 | code and probes landed. ⛔ **CORRECTED IN PLACE 2026-08-22 — the withdrawn wording read *"H9 meter, control-line and radio-load checks remain"*.** The **meter** check RAN on metal 2026-08-20 (walkthrough Phase 4a step 18 ☑, `battery vs multimeter … no percentage anywhere` = bench **8.6** / guide **H9-02**'s one-sided window). ⇒ what remains is the **control-line** arms (8.28 divider-not-conducting, 8.31 fail-safe park — guide H9-05 parts B/C) and the **radio-load** arm (8.30) |
| Status/navigation chrome | ✅ implementation · 🧪 metal | CHROME-1…CHROME-4 implemented the fixed top status strip, five-slot left navigation rail, settings badge and 19-column body. The detailed normative authority is `2026-08-15-heltec-mobile-status-navigation-ui-design.md`. ⛔ **CORRECTED IN PLACE 2026-08-22, WITHDRAWN NOT DELETED: this ended *"bench Parts 24–25 remain the visual, timing and sleep/radio acceptance gate"*. THE 24/25 SUBSET WAS RUN.** **24.1/24.2** on metal 2026-08-20 (walkthrough Phase 4a steps 17-18 ☑ — strip legibility, icon stability past `9`/`99`, and each slot checked against the console) and **25.1-25.5** (Phase 4b steps 19-23 ☑, the phase headed *"confirmed OK"* — one boxed rail icon, the modal mapping, the emergency full-width arm, the badge table, the 19-column body). The **24.3 / 25.6 sleep half** is `[x]` from the prior session's run sheet (`docs/superpowers/plans/2026-08-18-metal-session-run-sheet.md` §3 — one no-console boot discharges both). **25.4's `RESTART NEEDED` arm is N/A by build** — `MR_UI_BLE_ROW` is set by no env, so it is recorded **not-run with that reason**, ⛔ never a FAIL. ⇒ 🧪 **the measured residue is 25.7 alone**: the separate `gateway_heltec` non-team layout image (walkthrough Phase 8, optional, not run). |
| UI-7D inbox detail/delete | ✅ **UI + storage** · 🧪 metal | ⛔ **CORRECTED IN PLACE 2026-08-13: this row used to read `📝 UI · ✅ storage` and end *"§3.5's modal is slice B and is not built: the double press on INBOX still intentionally does nothing"*. THAT IS NOW FALSE.** **Slice A** (storage) landed 2026-08-06, QA-rejected the same day, re-closed 2026-08-07 — §6.2's `Inbox::erase(InboxKind, seq)` (tombstone; three outcomes; console `del_msg`), with the durable store's **mid-frame tear** ([[B135]], pre-existing) and the verb's **target parsing** ([[B136]]) both fixed. **Slice B** (the §3.5 modal) landed **2026-08-13**: `src/firmware_ui_model.h` owns the identity-tracked selection (`(InboxKind, seq)`), the modal, its paging and all three erase landings; `src/firmware_ui.cpp` owns the `(kind, seq)` lookup, the in-callback body copy and the one `erase()` call. 32 native mutations + 13 device-half probe controls, all RED. 🧪 **What remains is metal** — ⛔ **QUALIFIED IN PLACE 2026-08-22; the withdrawn wording read *"the panel's own rendering of the modal and the [[B134]] non-persistence check"*, and the RENDERING half is no longer owed.** The modal was rendered on real glass 2026-08-20 (walkthrough Phase 4b step 20 ☑ — INBOX stays boxed through detail + `MESSAGE GONE` — and step 23 ☑ — a ≥120-byte message paged in 19-column rows with `n/N` reaching the true tail): bench **25.2 / 25.5**. ⇒ what remains is bench **Part 19**'s own checks and the [[B134]] non-persistence control (`docs/2026-07-31-bench-test-script.md` §UI-7D), **plus the [[B231]]/[[B233]] list-behaviour recheck — which is ⛔ NOT done: it was DEFERRED BY OWNER RULING into the §UI-17 full-UI metal session** (spec `2026-08-20-ui17-navigation-status-team-redesign-spec.md` §7.5 absorbs bench Part 31 and `docs/superpowers/plans/2026-08-20-inbox-recheck-walkthrough.md`) |
...
| T2/T3 transmit-completion truth | ✅ implementation · 🧪 metal | HAL completion outcomes and app/UI `QUEUED` → `SENT` / `NO RELAY HEARD` semantics are implemented. ⛔ **CORRECTED IN PLACE 2026-08-22, THE WITHDRAWN WORDING KEPT VISIBLE: this read *"[[B164]] remains open for Part 22 metal correlation"*. PART 22 WAS RUN AND CONFIRMED OK ON METAL 2026-08-20** — walkthrough Phase 4c (`P 22.1-22.2`): step 24 ☑ `AIRED ctr=<n> dst=<id>` then `ACKED ctr=<n>` on a DM, step 25 ☑ the channel post's **full** handle (⛔ not its low byte, the §B40 truncation defect), step 27 ☑ the panel `SENDING...` → `SENT, waiting` → `PICKED UP` / `NO RELAY HEARD` with no `QUEUED` stall. ⚠ **Two arms of that Part carry no individual tick and the honest reading is the phase header plus the owner's *"whole walkthrough done, no more remarks"* (2026-08-20):** the relay-silence negative (step 26) and the peer-powered-off arm (step 28). ⚠ **T2's own residue is UNTOUCHED by this correction:** bench **Part 21** (the four TX diagnostics on `status`, and the BLE/JSON status contract that still omits `txfail`/`txoutdrop`/`txto`) is not recorded run. [[B189]]'s B186b recovery is still deliberately not implied by observability. |
| UI-13 typed staged-config service | ✅ **service only — HEADLESS** | ⛔ **CORRECTED IN PLACE 2026-08-13: this used to be one `UI-13…UI-16` row reading `📝 … no SETTINGS screen, draft marker, team-create/static-join UI or nearby-team onboarding exists yet`. That sentence is now FALSE for UI-13 and still TRUE for UI-14…UI-16, so the row is SPLIT rather than overwritten.** `src/firmware_config_service.h` implements §3.6.1: the three states (persisted / effective / draft) with `config_unsaved`, `conflict` and `reboot_required` as three DISTINCT comparisons, typed per-field validation, whole-candidate validation before any write, exactly ONE durable `/mrcfg` write, live apply only after durable success, DISCARD/RELOAD, and the two ruled headlines `CFG! RELOAD` / `SAVE FAILED`. Covered fields = the four already-durable ones (`ble_mode`, `e2e_dm`, `intro_attach`, `mobile_autoregister`); no NV schema change, no `kVersion` bump. 29 native cases / 32 mutations, all RED. ⚠ **Proved against FAKES: the NV fault / power-cut half is deferred with the device binding to UI-14 / [[B193]].** ⛔⛔ **CORRECTED IN PLACE 2026-08-13 (QG round 2): this row used to END *"NOTHING RENDERS OR CALLS IT: no SETTINGS screen, no menu row, no gesture, and `ICfgStore`/`ICfgLive` have no device binding yet, so the running firmware's behaviour is unchanged"* — SITTING DIRECTLY ABOVE THE UI-14 ROW THAT SAYS THE OPPOSITE. That sentence is withdrawn here, not deleted.** ★ **Read it as what it always was: the §UI-13 SLICE BOUNDARY.** UI-13 landed HEADLESS by scope — no renderer, no menu row, no gesture and no hardware binding *in that slice* — and **§UI-14 CONSUMES it**: the SETTINGS screen renders it, `src/firmware_ui.cpp` constructs the one `ConfigService`, and `mrfw::device_cfg_store()` / `device_cfg_live()` supply the binding ([[B193]]'s first half). ⇒ the running firmware's behaviour **is** changed by UI-14, and the header of `src/firmware_config_service.h` carries the same correction beside the obligations it recorded. ⛔ **What is STILL owed is [[B193]]'s other half:** the binding is exercised only through FAKES, so no real-flash write, no wear and no reset-during-write is proved by any gate — bench Parts 19/20 ⛔⛔ **CORRECTED IN PLACE 2026-08-22, THE WITHDRAWN WORDING KEPT VISIBLE: the two clauses *"no real-flash write"* and *"no reset-during-write is proved by any gate"* ARE FALSE.** Bench **Part 20.5 RAN AND PASSED** — the falsifier is run-sheet §9's `[x]` (`docs/superpowers/plans/2026-08-18-metal-session-run-sheet.md`) **plus the explicit owner confirmation of 2026-08-22 that the mark means ran-and-passed**; register **[[B193]] is CLOSED** on that record. ⓘ Its `/mrjoin` sibling was qualified separately, by the §UI-15 **slice-7 power-cut** (walkthrough Phase 7). ⚠ **KEPT QUALIFIED, deliberately: flash WEAR** — Part 20.5's own step 5 says *"nothing above proves flash WEAR"* — and the **nRF52/LittleFS arm**, since the bench nodes were ESP32/NVS (the sibling records that arm not-run with the hardware reason). ⓘ **Bench Part 19 (UI-7D) is untouched by this and stays owed.** |
| UI-14 SETTINGS screen, draft marker, save/discard/reboot | ✅ **UI + the device binding + the REAL `/mrcfg` power-cut** · 🧪 flash wear + the nRF52/LittleFS arm only (⛔ **mark corrected 2026-08-22**, it read *"🧪 NV / power-cut"*) | ⛔ **CORRECTED IN PLACE 2026-08-13: this was part of a `UI-14…UI-16 📝` row saying *"no SETTINGS screen, no draft marker on the panel"*. That is now FALSE for UI-14 and still TRUE for UI-15/UI-16, so the row is SPLIT rather than overwritten.** §3.6.2's menu is the fifth cycle slot on both cycles: the three live-class value rows with `short`-to-cycle / `double`-to-enter-and-accept, SAVE / DISCARD / BACK, a **conflict-conditional RELOAD** row, and a **present-but-inert PROVISION** row that refuses out loud — ⛔ **CORRECTED IN PLACE 2026-08-22: that last clause described the §UI-14 SLICE BOUNDARY and is FALSE as current behaviour.** Since §UI-15 the PROVISION row **activates** into the child menu (and is **hidden** when no child is available, owner ruling 2026-08-19); see the UI-15 row below. §3.3's marker is on STATUS (`CFG* UNSAVED` / `CFG! RELOAD`) and `RESTART NEEDED` is its own row. `src/firmware_ui_model.h` owns every gesture meaning, the row table and all four action landings; `src/firmware_ui.cpp` owns the frame-frozen render and the one `ConfigService` instance; **[[B193]]'s DEVICE BINDING landed in `src/firmware_config.cpp`** (`device_cfg_store` / `device_cfg_live` — the §nv-ritual `nv_load_stamped` load and the OFF→ON `mobile_register_current()` bridge). 29 native cases / 18 new model mutations / 8 new device-half probe controls. ⚠ **The BLE-mode row is ABSENT in every env in the tree** — §3.6.2's own condition, since UI-12 does not exist; the present arm is built and run by the probe under `-DMR_UI_BLE_ROW=1`. 🧪 **What remains is the storage half: no NVS/LittleFS write, no wear and no reset-during-write is exercised by any gate** — see `docs/2026-07-31-bench-test-script.md` §UI-14 ⛔⛔ **CORRECTED IN PLACE 2026-08-22, WITHDRAWN NOT DELETED: the *"no NVS write"* and *"no reset-during-write"* clauses ARE FALSE.** Bench **Part 20.5 RAN AND PASSED** on the real `/mrcfg` record — run-sheet §9 `[x]` **+ the explicit owner confirmation of 2026-08-22 that the mark means ran-and-passed**; **[[B193]] is CLOSED** on that record (its `/mrjoin` sibling was qualified separately by the §UI-15 slice-7 power-cut, walkthrough Phase 7). ⚠ **Two remainders are KEPT qualified because the run does not reach them: flash WEAR** (Part 20.5 step 5 says so in as many words) **and the nRF52/LittleFS write path** (the bench nodes were ESP32/NVS) ⛔ **AMENDED 2026-08-20 by [[B232]] (owner ruling): the fifth slot LANDS CLOSED on a single `ENTER SETTINGS` row — `short` passes in one press, `double` opens the menu (and stays CLOSED when the service cannot open). The auto-enter-on-arrival described above is WITHDRAWN.** The service still opens on arrival and the remedy words still render from the closed view. |
| UI-15 on-device provisioning — team create + static join | ✅ **COMPLETE, INCLUDING METAL** | ⛔⛔ **SPLIT AND CORRECTED IN PLACE 2026-08-22, AND THE WITHDRAWN WORDING IS KEPT VISIBLE: this was ONE `UI-15…UI-16 📝` row reading *"§3.6.3–§3.6.4; no team-create/static-join UI and no nearby-team onboarding exists. ⚠ The SETTINGS menu's PROVISION row is rendered and refuses; §3.6.3's 'an unsaved draft requires SAVE or DISCARD first' precondition is deliberately not implemented — it belongs to UI-15"*. EVERY ONE OF THOSE CLAUSES IS FALSE FOR UI-15**, so the row is SPLIT rather than overwritten (UI-16 below is unchanged). ✅ **The flows exist**: SETTINGS → `PROVISION` → `CREATE TEAM` / `JOIN NETWORK`, the `/mrjoin` four-profile store, `BACK`-first confirmations, and the `JOINING` / `STILL JOINING` / `ADOPTED` waiting screen. ★ **The precondition IS implemented, and as TWO cells with two remedies** — the `CfgRow::provision` arm in `src/firmware_ui_model.h` tests `conflict()` FIRST (`RELOAD OR DISCARD`) then `config_unsaved()` (`SAVE OR DISCARD`), opening nothing and ⛔ never saving on the operator's behalf; and the **parent PROVISION row is HIDDEN when no child is available** (owner ruling 2026-08-19 — `provision_has_child` / `settings_rows`, which is why `gateway_heltec` shows no PROVISION row at all). ★★ **METAL: PASSED 2026-08-20**, `docs/superpowers/plans/2026-08-20-metal-session-walkthrough.md` — Phase 3 (`P 28.1-28.5`, incl. the four-decimal store and the power-cycle), Phase 5 (`P 29`: create, `REPLACES`, the `PHY DIFFERS` divergence arm, durability), Phase 6 (`P 30`: the sparse profile list, `JOINING` → `ADOPTED`, the layer-17 discriminator, wake-press consumption, power-cycle and both DM directions) and Phase 7's `/mrjoin` **power-cut** (the slice-7 gate); owner: *"whole walkthrough done, no more remarks"*. ⓘ Two session residues, closed in code and carried in `docs/2026-07-30-open-bug-register.md`: [[B230]] (metal recheck = bench 27.17) and [[B232]] — both absorbed into the §UI-17 metal session (UI-17 spec §7.6/§7.7) |
| UI-16 nearby-team onboarding | 🚧 in progress (spec `docs/superpowers/specs/2026-08-22-ui16-nearby-onboarding-spec.md`) | §3.6.4; ✅ landed 2026-08-22/23: the `/mrteams` keyring (K1/K2), the observation cache (N1) and the read-only NEARBY scan (N2, incl. the F-1 `SAME RADIO + LEAF` correction above); 📝 remaining: confirmed join (N3), invite window (N4), REQUEST PUBKEY (N5), GRANT/REJECT (N6), grant-receive persistence (K3/K4/K5) |
| §UI-17 navigation, STATUS body, TEAM rows | ✅ **software arc COMPLETE** · 🧪 metal | **ROW ADDED 2026-08-22** — the arc had no row here at all while its corrections were landing throughout this document (§3.2, §3.3, §6, §10.3). Landed slices: **S1** TEAM/INBOX passive↔interactive navigation · **S2** §3.3 retention for the existing compose and detail modals · **S3** the fixed five-fact STATUS body beside the reserved 24x24 mark · **S4** the TEAM row's label + ROUTE/KNOWN age (hops and the never-rendered "signal quality" dropped) · **S5** distance/direction from the EXISTING authenticated peer-location cache (zero wire) · **S6** the real mark artwork · **S8** wake-on-receive. ⛔ **S7 / TEAM re-ordering is DEFERRED BY OWNER RULING** (spec §9 R-2). ★ **Normative authority = `docs/superpowers/specs/2026-08-20-ui17-navigation-status-team-redesign-spec.md`** and its per-slice dispatch plans (`docs/superpowers/plans/2026-08-2*-ui17-s*.md`) — ⛔ read slice state THERE, never from a count restated in this table. 🧪 **Metal = ONE full-UI walkthrough, that spec's §7** (bench Parts 32/33/34 are its per-slice residue), which by owner ruling also ABSORBS the [[B231]]/[[B233]] inbox recheck (§7.5), bench 27.17 / [[B230]] (§7.6) and the [[B232]] press sequences (§7.7) |
...
⛔ **COLUMN HEADER CORRECTED 2026-08-22: it read *"status on 2026-08-17"*, a single whole-table date stamp that was
already false for every row corrected after it — and that a sweep has to re-stamp to stay honest.** Each cell now
carries its own correction date instead; ⛔ **never re-freeze a table-wide "as of" here — the live records are
`docs/2026-07-30-open-bug-register.md`, `docs/2026-07-31-bench-test-script.md` and the per-arc specs/plans.**
...
### 3.1 Cycle · ✅ current cycle; ✅ provisioning extension (§UI-15) — ⛔ **corrected 2026-08-22, this heading read *"📝 provisioning extension"***
...
| 5 | SETTINGS / PROVISION | `MR_FEAT_OLED` | ✅ SETTINGS (§UI-14, 2026-08-13, landing CLOSED since [[B232]]); ✅ PROVISION (§UI-15) — see the correction below this table |
...
⛔⛔ **CORRECTED IN PLACE 2026-08-22 (§UI-15 + [[B232]]), AND THE WITHDRAWN CELL IS KEPT VISIBLE — slot 5 read
*"✅ SETTINGS (§UI-14, 2026-08-13); 📝 PROVISION (§UI-15) — the row exists and refuses"*. BOTH HALVES OF THAT ARE NOW
WRONG.** **(a) PROVISION does not refuse — §UI-15 landed the flows**, and the `CREATE TEAM` / `JOIN NETWORK` children
were exercised on real hardware on 2026-08-20 (`docs/superpowers/plans/2026-08-20-metal-session-walkthrough.md`
Phases 5 and 6). **(b) The slot LANDS CLOSED** on a single `ENTER SETTINGS` row ([[B232]], owner-ruled): `short`
passes the screen in one press, `double` opens the menu — and the **parent PROVISION row is HIDDEN whenever no child
is available** (owner ruling 2026-08-19; `provision_has_child` / `settings_rows` in `src/firmware_ui_model.h`), which
is why a `gateway_heltec` image shows **no PROVISION row at all** (walkthrough Phase 8).
...
### 3.6 On-device settings and provisioning (owner extension 2026-08-06) · ✅ implemented, EXCEPT §3.6.4's nearby onboarding

⛔ **HEADING CORRECTED IN PLACE 2026-08-22, WITHDRAWN NOT DELETED: it read *"📝 not implemented, EXCEPT §3.6.1's
service"*.** That was true on 2026-08-13 for one slice and has been false since: §3.6.1's service (§UI-13), §3.6.2's
SETTINGS menu (§UI-14) **and §3.6.3's provisioning operations (§UI-15, metal-qualified 2026-08-20 —
`docs/superpowers/plans/2026-08-20-metal-session-walkthrough.md` Phases 5-6)** have all landed. **§3.6.4 alone is
still 📝** (UI-16). ⛔ **CORRECTED IN PLACE 2026-08-22 — this sentence closed *"and [[B193]]'s real-flash / power-cut
qualification is still open for the storage half"*, which held for one day: Part 20.5 RAN AND PASSED** (run-sheet §9
`[x]` + the explicit owner confirmation of 2026-08-22 that the mark means ran-and-passed; **[[B193]] CLOSED**).
⚠ Of the storage half only **flash wear** and the **nRF52/LittleFS** arm stay unmeasured — see the UI-13/UI-14 rows.
...
⛔ **CORRECTED IN PLACE 2026-08-13 (QG round 2): this heading read *"implemented … (§UI-13) — the SERVICE only, headless"*, which was true for one slice and is now misleading — §UI-14 landed the renderer AND the `ICfgStore`/`ICfgLive` device binding. ⚠ Still true and NOT corrected away: [[B193]]'s real-flash / power-cut qualification is OPEN.** ⛔ **THAT LAST CLAUSE IS NOW WITHDRAWN TOO — CORRECTED IN PLACE 2026-08-22: bench Part 20.5 RAN AND PASSED** (run-sheet §9 `[x]` + the explicit owner confirmation of 2026-08-22 that the mark means ran-and-passed; **[[B193]] CLOSED**; the `/mrjoin` sibling was qualified separately by the §UI-15 slice-7 power-cut). ⚠ **Only flash wear and the nRF52/LittleFS arm remain unmeasured.**
...
NV / power-cut qualification remains deferred to the UI-14 device binding and [[B193]].** ⛔ **CORRECTED IN PLACE
2026-08-22: that deferral is DISCHARGED — bench Part 20.5 ran and passed** (run-sheet §9 `[x]` + the explicit owner
confirmation of 2026-08-22 that the mark means ran-and-passed; **[[B193]] CLOSED**). ⚠ The ruling's own scope is
unchanged, and flash wear / the nRF52/LittleFS arm stay unmeasured.
...
as a snapshot field so both arms are tested. **(b)** ⛔⛔ **CORRECTED IN PLACE 2026-08-22 (§UI-15), AND THE WITHDRAWN
WORDING IS KEPT VISIBLE — this read *"the **PROVISION row is PRESENT and INERT**: it is a row of this menu, so it is
rendered, and activating it refuses out loud (`PROVISION: UI-15`). ⛔ §3.6.3's 'if a settings draft is unsaved,
PROVISION first requires SAVE or DISCARD' is **not** implemented — it is a rule about a flow that does not exist"*.
ALL THREE CLAIMS ARE FALSE.** The row **activates** into the §3.6.3 child menu (`enter_provision(Provision::menu)`,
`src/firmware_ui_model.h`'s `CfgRow::provision` arm), it is **CONDITIONAL — hidden when no child is available**
(owner ruling 2026-08-19, `provision_has_child`), and the precondition **is** implemented as **two cells with two
remedies**: `conflict()` is tested FIRST ⇒ `RELOAD OR DISCARD` (⛔ never SAVE — `save()` refuses a conflict), then
`config_unsaved()` ⇒ `SAVE OR DISCARD`; both open nothing and ⛔ never save on the operator's behalf. Both cells were
exercised on metal 2026-08-20 (walkthrough step 37 ☑). **(c)** a **RELOAD row appears exactly while `conflict()` stands**: §3.6.1 requires a conflict to be resolved by
...
#### 3.6.3 Provisioning operations · ✅ implemented 2026-08-19/20 (§UI-15 slices 1-7) and metal-qualified 2026-08-20

ⓘ **STATUS ADDED 2026-08-22:** this heading carried no status while §3.6.1 and §3.6.2 carried theirs, so a reader
consulting it inherited the (now corrected) *"📝 not implemented"* of §3.6's own heading. The flows, the `/mrjoin`
store and the waiting screens are built and were run on hardware — `docs/superpowers/plans/2026-08-20-metal-session-walkthrough.md`
Phases 3, 5, 6 and 7. Read slice-level state from the register and that walkthrough, ⛔ not from a tally here.
...
2. The joiner listens on its **current effective PHY**, collects recently observed non-zero team ids and shows a
   ...
   > ⛔ **CORRECTED IN PLACE 2026-08-23 (§UI-16 N2, finding F-1), AND THE ORIGINAL WORDING ABOVE IS KEPT VISIBLE.**
   > *"Listens on its current effective PHY"* is **TRUE but INCOMPLETE — the LEAF NIBBLE gates discovery as well,
   > and it does so BEFORE parse.** Measured in `Node::ingest_beacon` (`lib/core/node_beacon.cpp:558`):
   > `if (wire::flags_of(bytes[0]) != _cfg.leaf_id && _cfg.team_id == 0) return;` ⇒ a **TEAMLESS** joiner — exactly
   > the node this flow serves — **drops every beacon whose leaf nibble differs from its own**, on the right
   > frequency, bandwidth and spreading factor. Two nodes can be in earshot, correctly tuned, and hear nothing of
   > each other.
   > ⛔ **§UI-16 does NOT relax that gate**: relaxing a pre-parse drop is a routing-plane change with corpus
   > consequences, and it is not onboarding. ⇒ **the panel says so instead**: the scan screen carries
   > `CURRENT PHY ONLY` **and a second line, `SAME RADIO + LEAF`** (§UI-16 string S-4, landed in N2), so an operator
   > whose peer sits on another leaf reads the reason rather than concluding the other node is off.
   > ⓘ The **PHY boundary** paragraph below inherits this: *"discovers teams already audible on the current PHY"*
   > must be read as *"…on the current PHY **and the current leaf nibble**"*.
...
## 7. Battery reader · ✅ code landed; 🧪 H9 hardware qualification PARTLY discharged

ⓘ **QUALIFIED 2026-08-22 — the heading read *"🧪 H9 hardware qualification remains"*, which is now too wide.** The
**meter** arm ran on metal 2026-08-20 (`docs/superpowers/plans/2026-08-20-metal-session-walkthrough.md` Phase 4a
step 18 ☑ = bench 8.6 / guide H9-02, one-sided window, no percentage anywhere). ⛔ **Still owed:** the control-line
arms (8.28 / 8.31 — the divider must not conduct between samples, and the refusal path's fail-safe park) and the
radio-load arm (8.30). Read the current state from the bench script, not from this heading.
...
| UI-13 | ✅ **service only — HEADLESS** · 🧪 flash wear + nRF52/LittleFS (⛔ **corrected 2026-08-22**, read *"🧪 NV/power-cut"*) | typed staged-config service, conflict detection and one-write commit (§3.6.1) | ⛔ **CORRECTED IN PLACE 2026-08-13: this cell read `📝` and the "Recommended next order" line below still named UI-13 as the next slice to START. Both were true until the service landed and are now false. FOURTH instance in this arc of a correction reaching the prose but not the table a reader acts on — which is why the sweep for this one covered §13's table, §13's next-order sentence, the §status map and §3.6/§3.6.1's headings.** ✅ **landed 2026-08-13** (`src/firmware_config_service.h`): three states, `config_unsaved` / `conflict` / `reboot_required` as three distinct comparisons, typed validation, whole-candidate validation before any write, ONE durable write, live apply only after durable success, DISCARD/RELOAD, `CFG! RELOAD` / `SAVE FAILED`. Native: 29 cases, **32 mutations all RED**. ⛔⛔ **CORRECTED IN PLACE 2026-08-13 (QG round 2): this cell read *"HEADLESS — no screen, no cycle change and NO DEVICE BINDING (`ICfgStore`/`ICfgLive` unimplemented on hardware)"*. THAT IS NOW FALSE and is withdrawn here, not deleted — it describes the §UI-13 SLICE BOUNDARY, and §UI-14 (the row below) landed BOTH the screen and the binding.** ★ Read as history: UI-13 shipped headless by scope; UI-14 consumes it. ⇒ 🧪 **the gate's "NV fault/power-cut" half is STILL NOT met — it moved to [[B193]] with the binding rather than being discharged by it**: everything here is proved against a counting/failing FAKE store, which measures the LOGIC and cannot measure flash, wear or a reset mid-write ⛔⛔ **CORRECTED IN PLACE 2026-08-22, AND THE WITHDRAWN CLAIM IS KEPT VISIBLE: *"the gate's NV fault/power-cut half is STILL NOT met"* IS FALSE for the power-cut.** Bench **Part 20.5 RAN AND PASSED** against the real store — run-sheet §9 `[x]` **+ the owner's explicit 2026-08-22 confirmation that the mark means ran-and-passed**; **[[B193]] CLOSED** (the `/mrjoin` sibling was qualified separately by the §UI-15 slice-7 power-cut). ⚠ **The FAKE-store sentence itself stands as written — it describes what THIS slice's gate measures**, and two of its three items are still unmeasured anywhere: **wear** (Part 20.5 step 5 disclaims it) and the **nRF52/LittleFS** path (ESP32/NVS bench). Only *"a reset mid-write"* is now covered, and by metal rather than by this gate |
| UI-14 | ✅ **UI + device binding + the REAL `/mrcfg` power-cut** · 🧪 flash wear + nRF52/LittleFS (⛔ **corrected 2026-08-22**, read *"🧪 NV/power-cut"*) | SETTINGS screen, marker and save/discard/reboot states (§3.6.2) | ⛔ **CORRECTED IN PLACE 2026-08-13: this cell read `📝` and the gate column read *"native + board probe + target"*. ✅ landed 2026-08-13** — the fifth cycle slot on both cycles, the row table with its two CONDITIONAL rows (BLE by the UI-12 transport, RELOAD by `conflict()`), `short`'s two modes behind the `Settings{closed,browsing,editing}` state, `double` to enter/accept, SAVE / DISCARD / RELOAD / BACK through the §UI-13 service, §3.3's three literals on STATUS, the `long_arm` editor close, and **[[B193]]'s device binding** in `src/firmware_config.cpp`. **PROVISION is present-but-inert; §3.6.3's unsaved-draft precondition is NOT built.** ⛔ **CORRECTED IN PLACE 2026-08-22: BOTH HALVES OF THAT SENTENCE ARE NOW FALSE — it described §UI-14's slice boundary.** §UI-15 landed the flows (the row activates, and is hidden when no child is available), and the precondition IS built as two cells with two remedies (`conflict()` → `RELOAD OR DISCARD`, then `config_unsaved()` → `SAVE OR DISCARD`), exercised on metal 2026-08-20 (walkthrough step 37 ☑). 18 new model mutations + 8 new probe controls, both UI probes green, census at its pins, 36/36 corpus with 0 movers. ⛔ **NATIVE TOTAL CORRECTED IN PLACE 2026-08-13 (§notify-every-save): this cell published `1610/82310/0`, which was already stale by the §UI-14 FOLLOW-UP's `1613/82339/0` and is now stale again. The live figure is `1615/82362/0`** — and the lesson is that a suite-wide total published in a per-row cell goes stale on the next slice by construction, so read it from the tree, never from here. 🧪 **THE `target` HALF IS NOT MET: no NVS/LittleFS write, no wear and no reset-during-write is exercised by any gate** — the bindings are proved only by compiling, and everything behavioural runs against FAKES. That is a BENCH check (`docs/2026-07-31-bench-test-script.md` §UI-14) and [[B193]] keeps its 🧪 until it runs ⛔⛔ **IT RAN — CORRECTED IN PLACE 2026-08-22, WITHDRAWN NOT DELETED.** Bench **Part 20.5 PASSED**: run-sheet §9 `[x]` **+ the explicit owner confirmation of 2026-08-22 that the mark means ran-and-passed** ⇒ **[[B193]] CLOSED**, the real NVS write and the reset-during-write are both proved on metal, and the `/mrjoin` sibling was qualified separately by the §UI-15 slice-7 power-cut. ⚠ **Kept qualified, because the run cannot reach them: flash WEAR** (Part 20.5 step 5) **and the nRF52/LittleFS write path** (ESP32/NVS bench nodes) ⛔ **AMENDED 2026-08-20 by [[B232]] (owner ruling): SETTINGS now LANDS CLOSED on a single `ENTER SETTINGS` row — `short` passes in one press, `double` opens the menu (closed-stay when the service cannot open); menu BACK/walk-off return to the closed view. The auto-enter/walk-to-leave described above is WITHDRAWN.** |
| CHROME-1…4 | ✅ implementation · 🧪 metal | status strip, navigation rail, settings badge and 19-column migration | ⛔ **CORRECTED IN PLACE 2026-08-22 — this cell read *"implementation complete; bench Parts 24–25 remain"*.** The 24/25 subset RAN on 2026-08-20 (24.1/24.2 + 25.1-25.5, walkthrough Phases 4a/4b); the 24.3/25.6 sleep half is `[x]` from the 2026-08-18 run sheet; 25.4's `RESTART NEEDED` arm is N/A by build. ⇒ **residue = 25.7's separate `gateway_heltec` image**. Full detail in the §status-map row |
| UI-15 | ✅ **COMPLETE, INCLUDING METAL** | atomic team creation and four-profile static join (§3.6.3) | ⛔ **CORRECTED IN PLACE 2026-08-22: this row read `📝` with the gate column *"native + NV fault/power-cut + multi-node metal"* — i.e. the second table a reader consults to answer "is this built?" still said UI-15 was not, while §3.6.3 and the §status map had been corrected. That is the Class-4 shape this document has now hit for the FIFTH time (a correction placed anywhere except the row a reader acts on).** ✅ **All three named gates RAN:** the native suite behind slices 1-7; the **`/mrjoin` power-cut** on real flash (walkthrough Phase 7, the slice-7 gate); and **multi-node metal** — Phases 3/5/6 across H1, H2 and X (`P 28`, `P 29`, `P 30`), owner 2026-08-20: *"whole walkthrough done, no more remarks"* |
| UI-16 | 🚧 **in progress** — normative authority = `docs/superpowers/specs/2026-08-22-ui16-nearby-onboarding-spec.md`; ⛔ read slice state there, not from this table | current-PHY nearby-team scan, explicit candidate approval and sealed key grant (§3.6.4) | native + RF isolation controls + multi-node metal · **ROW UPDATED 2026-08-23:** K1/K2 (`/mrteams` keyring, = [[B240]]), N1 (observation cache) and N2 (the read-only NEARBY scan, with the §3.6.4 F-1 leaf-nibble correction) are landed and QG-passed; the metal residue is bench Parts 35-36; approval (N3-N6) and grant-receive persistence (K3-K5) remain 📝 |
| §UI-17 | ✅ **software arc COMPLETE** · 🧪 metal | navigation migration, §3.3 retention, the STATUS body + 24x24 mark, TEAM rows, location projection and wake-on-receive (S1-S6, S8) | **ROW ADDED 2026-08-22.** ⛔ S7 / TEAM ordering **deferred by owner ruling** (spec §9 R-2). Normative authority = `docs/superpowers/specs/2026-08-20-ui17-navigation-status-team-redesign-spec.md` + its per-slice plans — ⛔ read slice state there, not from this table. 🧪 **Metal = the one full-UI walkthrough in that spec's §7**, which also absorbs the deferred [[B231]]/[[B233]] recheck, bench 27.17 / [[B230]] and the [[B232]] press sequences |
...
**UI-7D is a hardware gate too now — its firmware is complete and only bench Part 19 is outstanding.** ⚠ **QUALIFIED 2026-08-22: *"only bench Part 19"* is one item short.** The modal's rendering was exercised on glass 2026-08-20 (bench 25.2 / 25.5), but the [[B231]]/[[B233]] **list-behaviour recheck (bench Part 31) is also outstanding** — deferred BY OWNER RULING into the §UI-17 full-UI metal session (UI-17 spec §7.5), not discharged.
**UI-10…UI-12 and UI-16 are extensions and must not be reported as current behavior.**
⛔⛔ **CORRECTED IN PLACE 2026-08-22 — this sentence read *"UI-10…UI-12 and UI-15…UI-16 are extensions"*, which stopped
being accurate when §UI-15 landed (slices 1-7) and was metal-qualified on 2026-08-20
(`docs/superpowers/plans/2026-08-20-metal-session-walkthrough.md`). It is the sentence a reader acts on, so it is
withdrawn here, not deleted. UI-16 keeps the prohibition; UI-15 does not.**
...
⛔⛔ **AND THAT 2026-08-13 CORRECTION IS ITSELF STALE IN ONE HALF — CORRECTED IN PLACE 2026-08-22: *"no provisioning
behaviour exists (the PROVISION row refuses)"* IS FALSE.** §UI-15 landed the CREATE TEAM / JOIN NETWORK flows and the
`/mrjoin` store, the row activates instead of refusing, and both were run on hardware (walkthrough Phases 5-6).
⚠ **The OTHER half stands unchanged and is not corrected away: the binding's NV / power-cut behaviour is still
unproven** ([[B193]], bench Part 20.5 — carried as owed in `docs/2026-07-30-open-bug-register.md`), so *"the storage
half is qualified"* must still not be reported. ⛔⛔ **THAT SENTENCE SURVIVED ONE DAY — CORRECTED IN PLACE 2026-08-22:
bench Part 20.5 RAN AND PASSED** (run-sheet §9 `[x]` + the explicit owner confirmation of 2026-08-22 that the mark
means ran-and-passed; **register [[B193]] CLOSED**; the `/mrjoin` sibling qualified separately by the §UI-15 slice-7
power-cut). ⇒ the real write and the reset-during-write ARE proved on metal; ⚠ what may still not be reported is the
narrower pair the run cannot reach — **flash wear** and the **nRF52/LittleFS** write path.
...
⛔⛔ **THAT ORDER IS SPENT — CORRECTED IN PLACE 2026-08-22, kept visible because it is an instruction a reader would
follow.** Of the gates it names: [[B196]]'s soak **CLOSED** on 2026-08-17 (register), [[B164]]/Part 22 and the
status/navigation visual checks **RAN** on 2026-08-20 (walkthrough Phases 4a-4c), and **UI-15 was then implemented in
full and metal-qualified in the same session** — so *"proceed UI-15"* is discharged, not pending. ⚠ **What it names
that is still owed stands:** ⛔ **AMENDED SAME-DAY 2026-08-22 — this listed *"[[B193]]'s real-storage / reset
behaviour (Part 20.5)"* among the owed items; it is NOT owed.** Part 20.5 ran and passed (run-sheet §9 `[x]` + the
explicit owner confirmation of 2026-08-22 that the mark means ran-and-passed; **[[B193]] CLOSED**), leaving only flash
wear and the nRF52/LittleFS arm unmeasured. ⇒ **still owed: bench Part 19** and the **[[B189]]**
recovery decision (⛔ still never to be inferred from T2/T3 observability). ★ **Do not re-derive a next order here —
the live queue is `docs/2026-07-30-open-bug-register.md` §"Recommended execution order" plus the §UI-17 spec's §7
metal session.**
