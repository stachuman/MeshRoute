...
The toggles default **off**, and the location flag/slot is added **only** when a toggle is on AND the location is non-zero. So a single-layer node with no location config never emits the flag → **s18 must stay BYTE-IDENTICAL**. The gate is the **meshroute** s18 (`/tmp/s18_mr.json`, the `engine:"meshroute"` variant of `s18_singlelayer_dense`): **157373 events / md5 `306c3cf4af65b56d6fc6415b964ad9f3`**. `pack_unicast_inner`'s new params, when the flag is absent, must produce the exact same bytes. This is the keystone — verify before/after. (NB: `77205506…` / 779015 events is the **Lua-engine** s18 — a *different* scenario, trivially unaffected by a C++-firmware change — NOT this gate.)
