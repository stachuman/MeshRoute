...
| `DATA_TYPE_E2E_ACK` | `0x80` | 3 | internal outcome | live · a `persistent_outcome` since Slice A; ⚠ **no longer the ONLY one** — §CUSTODY-G made §7.1's opt-in set `{E2E_ACK, CUSTODY_FAILURE}`. Producer: any node acking a `-a` DM · Receiver: `node_mac_rx.cpp`'s E2E-ACK arm (`record_ack` + `send_e2e_acked`) · Relay: forwarded as ordinary DATA · Persistence: YES · Outcome owner: itself · Coverage: `test_node_e2e_ack.cpp`, `§CUSTODY-C/5` |
| `DATA_TYPE_CUSTODY_FAILURE` | `0x81` | — | internal outcome | ★ **ALLOCATED BY §CUSTODY-F, COMPLETED BY §CUSTODY-G (both 2026-08-31)**. ⛔ THE STATUS THAT STOOD HERE — *"`persistent_outcome=false` until Slice G's storing consumer lands"* — IS NOW FALSE and is recorded rather than deleted: G landed the consumer and flipped the bit in the same slice. Producer: the selected transit cascade terminal (`custody_notice_enqueue`) · Receiver/consumer: `Node::custody_failure_receive` — §13's eighteen validations, then §7.3's record-before-push; an invalid record is dropped with the bounded scalar `custody_failure_reject` · Relay treatment: forwarded as ordinary transit DATA (a relay is not the addressee; only `failed_origin == self` consumes) · Persistence: **YES** (`Inbox::record_custody_failure`, §7.2's mapping) · Outcome owner: `PushKind::custody_failure` + the pulled `ev:custody_failure`; ⛔ never the generic send lifecycle · Coverage: `test_custody_relay_f.cpp`, `test_custody_receive_g.cpp` (38 cases), `§CUSTODY-C/2e` + `§CUSTODY-C/5b`, and 11 corpus receipts across four streams |
