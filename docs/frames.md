...
| ---- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | `H_ANSWER`                      | standard plaintext-unicast envelope `[origin 1]`, then body `[target_layer 1][node_id 1][key_hash32 4 LE]` (7-B inner; 20-B complete DATA)                                                                                 |
| 2    | `AUTHORITATIVE_H_ANSWER`        | same 7-B canonical envelope/body as `H_ANSWER`; the answer is the owner's (authoritative); 20-B complete DATA                                                                                                               |
...
| 5    | `AUTHORITATIVE_H_ANSWER_PUBKEY` | standard plaintext-unicast envelope, body `[target_layer 1][node_id 1][ed_pub 32][name_len 1][name 0..32]` — **the v1-emitted pubkey answer** (owner-authoritative); unchanged by B161; 49..81-B complete DATA without optional `DST_HASH` |
...
| 8    | `MOBILE_H_ANSWER`               | standard plaintext-unicast envelope `[origin 1]`, then body `[target_layer 1][node_id=home 1][key_hash32=M 4 LE][epoch 1]` (8-B inner; 21-B complete DATA) — registrar-proxy answer (§mobile §4a)                         |
...
| 13   | `MOBILE_H_ANSWER_PUBKEY`        | standard plaintext-unicast `[origin 1]`, then mobile hash-bind `[target_layer][home][key_hash32][epoch]` (7 B) ‖ `ed_pub[32]` ‖ `[name_len 1][name 0..32]`; 41..73-B inner, 54..86-B complete DATA. Cache `peer_key(M)`+`mobile_home(M→home)`, **never** id_bind the local id |
