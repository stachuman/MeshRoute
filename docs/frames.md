...
| b0 `0x01` | `PRIORITY` / `MS_ENCLOSED_TYPE` | ★ **TWO MEANINGS ON ONE BIT — NOT a free bit.** As `PRIORITY` it is decoded-only (`o.priority`; nothing reads it). But the **same bit** is `DATA_FLAG_MS_ENCLOSED_TYPE` (an alias, `frame_codec.h:704` — line refs corrected 2026-08-29, §CUSTODY-A) and is **LIVE**: on a same-layer MOBILE_SEND wrapper it marks an enclosed TYPE byte at `body[0]`. Set `node_channel.cpp:811`, `node_hashlocate.cpp:1742/1754`; read `node_mac_rx.cpp:1581/1638`. Disambiguated by frame context (wrapper vs ordinary DM), not by another bit. ⚠ **Do not repurpose it** — see the flags-byte-full note below |

**TYPE (byte 8, enum, present iff `APP`):** mutually-exclusive message kinds. Values are a **range contract**, not
allocation order (§CUSTODY-A, 2026-08-29 — every pre-existing ordinal renumbered; historical ordinals 1..19 appear
only in fenced history):

```
0x00        ordinary untyped DM — no TYPE byte is emitted (APP=0)
...
The internal-range predicate is exactly `t >= 0x80 && t <= 0xBF` — **not** `t & 0x80`, which would admit
`0xC0..0xFF`. Gaps inside `0x80..0xBF` are deliberate (`0x80..` core outcomes · `0x88..` hash/key discovery ·
`0x90..` mobility · `0xA0..` administration/security): a new member **appends inside its block** rather than
renumbering an existing one. `protocol::wire_version` is **unchanged at 1** — see `docs/protocol.md` §2.4 for the
mixed-firmware consequence. (`AUTHORITATIVE` remains folded into the H-answer code — `0x88` vs `0x89`; the old
`E2E_IS_ACK` flag became the `E2E_ACK` type. ⓘ This table previously omitted rows for `MOBILE_SEND` and
`TEAM_KEY_GRANT` — a pre-existing gap, filled 2026-08-29.)

| code | type | inner shape |
|---|---|---|
| `0x01` | `INTRO` | a PLAINTEXT app DM whose body is prefixed `[ed_pub 32][name_len u8][name ≤32]` before the text (requires SOURCE_HASH; receiver checks `ed_pub[:4]==source_hash`, caches authoritative, STRIPS the prefix, delivers the rest as a normal DM). Auto-attached on a first-contact plaintext hash-send until a sealed frame from that peer confirms custody (§S2 D1; `cfg set intro_attach` opt-out). Delegated same-layer via the `MS_ENCLOSED_TYPE` wrapper marker (aliases the decode-only PRIORITY bit — 1-hop wrapper leg only, stripped at the home) |
| `0x02` | `MOBILE_SEND` | the delegation wrapper: a hosted mobile's logical send carried to its HOME, which unwraps and re-originates under its own identity. Same-layer form marks an enclosed TYPE byte at `body[0]` via `MS_ENCLOSED_TYPE` (flags b0); cross-layer form always carries the enclosed type after the XL prefix. The enclosed type may itself be any DATA type (offset derivation: 9 + [dst_hash 4] + [XL prefix] + origin 1 + [source_hash 4]) — filled 2026-08-29, previously missing from this table |
| `0x03` | `SEALED_RELAY` | `[seal_ctr 2 LE][seed8 8][ct‖tag]` — a PLAINTEXT-framed sealed-body carrier (clear DST_HASH + SOURCE_HASH; the CARRIED `seal_ctr` drives the nonce so a delegating home re-originates under its OWN frame ctr without re-sealing). Directed open by the clear SOURCE_HASH (no trial); sealed-vs-clear source_hash cross-checked. Serves delegated same-layer AND cross-layer sealed DMs (§S4). ⚠ attributable envelope — sealed CONTENT, visible sender; sealed-SENDER privacy is the same-layer direct path only |
| `0x04` | `CHANNEL_POST` | enclosed-channel marker inside a MOBILE_SEND wrapper: `[channel_id 1][text]` — a registered mobile's GLOBAL channel post, delegated; the home unwraps and mints via `do_send_channel` under its OWN origin/ctr (§S7; anti-spam bills the home) |
| `0x05` | `APP_MESSAGE` | **RESERVED, unimplemented** — claimed for the app-code design (`2026-08-05-channel-app-code-draft.md`); no producer, no consumer; `known=false` in `data_type_traits()` until that design lands |
| `0x80` | `E2E_ACK` | normal-unicast inner, `body` = the acked `ctr` (2 B LE). **§mobile reverse-ack:** carries `SOURCE_HASH` = the acked DM's originator hash when the sender ≠ its `origin` (a mobile — `key_hash_of_id(origin) != sender_hash`), so the ack (routed to `origin` = the mobile's `home_id`) is recognized at the home and **last-miled to the mobile**; a static→static ack has `source_hash == key_hash_of_id(origin)` → no `SOURCE_HASH` (byte-identical) |
| `0x81` | `CUSTODY_FAILURE` | **LIVE since §CUSTODY-F (2026-08-31** — this row read *"reserved / not allocated yet"* until then**)**: a relay reports it could not complete onward custody for a transit DATA it had ACKed. Standard plaintext unicast inner; `Plane::GLOBAL` explicit; fresh reporter counter; no `E2E_ACK_REQ`, no `CRYPTED`. Body = the v1 custody record (`pack_custody_failure`/`parse_custody_failure` — the ONE codec): `[version=1][record_len 24..][notice_flags][terminal_reason][failed_origin][failed_dst][failed_ctr 2LE][failed_type][failed_data_flags][failed_plane][reporter_layer][previous_hop][failed_next_hop][requeue_count][alternatives_tried][committed_hops][remaining_hops][dst_hash32 4LE][reserved 2=0]` (24-B fixed prefix; a v1 reader stores a validated longer tail, interprets 24). `notice_flags`: b0 forwarded(=1) · b1 failed_at_cts · b2 failed_at_ack (exactly one of b1/b2) · b3 repair_attempted · b4 next_was_one_way · b5 has_dst_hash. Receivers CONSUME it before ordinary DM delivery (§CUSTODY-G, 2026-08-31 — this cell previously warned of the pre-G drop); the validation list and outcome contract are `docs/protocol.md`'s, not this file's |
| `0x88` | `H_ANSWER` | standard plaintext-unicast envelope `[origin 1]`, then body `[target_layer 1][node_id 1][key_hash32 4 LE]` (7-B inner; 20-B complete DATA) |
| `0x89` | `AUTHORITATIVE_H_ANSWER` | same 7-B canonical envelope/body as `H_ANSWER`; the answer is the owner's (authoritative); 20-B complete DATA |
| `0x8A` | `H_ANSWER_PUBKEY` | `[target_layer 1][node_id 1][ed_pub 32]` — **reserved, not emitted in v1** |
| `0x8B` | `AUTHORITATIVE_H_ANSWER_PUBKEY` | standard plaintext-unicast envelope, body `[target_layer 1][node_id 1][ed_pub 32][name_len 1][name 0..32]` — **the v1-emitted pubkey answer** (owner-authoritative); unchanged by B161; 49..81-B complete DATA without optional `DST_HASH` |
| `0x90` | `MOBILE_H_ANSWER` | standard plaintext-unicast envelope `[origin 1]`, then body `[target_layer 1][node_id=home 1][key_hash32=M 4 LE][epoch 1]` (8-B inner; 21-B complete DATA) — registrar-proxy answer (§mobile §4a) |
| `0x91` | `MOBILE_BREADCRUMB` | `body [new_home_id 1][new_epoch 1][new_home_layer 1]` (3 B), rides `SOURCE_HASH`=M — **NEW-home→old-home** on a re-home (§S6/D10; was mobile-sent pre-S6). Same-layer today; cross-layer deferred (`presence_notify_xl_deferred`) |
| `0x92` | `MOBILE_LAYER_QUERY` | empty body — a mobile asks a gateway "list the layers you bridge" (§mobile §5a, `SOURCE_HASH`=M) |
| `0x93` | `MOBILE_LAYER_ANSWER` | `[count u8][ count × LayerRecord ]` — a gateway's layer directory (§mobile §5a) |
...
**② Hash-bind answer** (`TYPE = H_ANSWER` / `AUTHORITATIVE_H_ANSWER`; cleartext, 7-B inner):
the standard envelope's `[origin 1 B]`, followed by the unchanged body `[target_layer 1 B] [node_id 1 B] [key_hash32 4 B LE]`. The `H_ANSWER` / `AUTHORITATIVE` distinction rides the frame TYPE (1 vs 2), **not** the body. The pubkey variant (`TYPE = AUTHORITATIVE_H_ANSWER_PUBKEY`, code 5) already uses the same envelope and replaces `key_hash32` with `ed_pub[32]`, then appends `[name_len][name]`.
...
- **Hash-bind** (`TYPE = H_ANSWER` / `AUTHORITATIVE_H_ANSWER`): canonical inner `origin`(1) · body `target_layer`(1) · `node_id`(1) · `key_hash32`(4 LE). Authoritative = the TYPE code (2 vs 1).
- **Pubkey** (`TYPE = AUTHORITATIVE_H_ANSWER_PUBKEY`, code 5): canonical inner `origin`(1) · body `target_layer`(1) · `node_id`(1) · `ed_pub`(32) · `name_len`(1) · `name`(0..32) (no `key_hash32` — it is `ed_pub[:4]`). **Mutual:** the owner first **caches the requester's `ed_pub`** (from the request) so it can decrypt the requester's sealed DMs + address this answer, then replies — **one round provisions BOTH directions** (the E2E bootstrap). The request rides the cleartext flood ⇒ both pubkeys are relay-visible: the deliberate "establishing contact" exposure; every DM after is sealed.
