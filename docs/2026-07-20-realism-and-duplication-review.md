...
## Wave-4 Slice-C investigation (2026-07-22, QA) — ★ do NOT ratify Slice C references in isolation

Slice C (per-node/link/path-loss RNG decoupling) reshuffled delivery; the two flagged tails were root-caused. **Both are REAL mechanisms, not seed fragility** — each a known weakness class exposed *because* decorrelated per-node timing is more metal-realistic (the correlated shared-mt19937 was masking them). Investigation is Phase-1 (root cause) only; no code changed.

- **s19 8/8→5/8 (deterministic, LOSSLESS, single-layer) = relay contention-starvation.** All 3 stuck DMs are B→A (3/3 directional). The relay on B's reverse path (an A-side neighbour) never acquires a route to A via **either** DV beacons **or** reactive RREQ — it is reception-starved (`beacon_rx` **6→4** vs 7–9 for peers). **A/B-confirmed:** revert Slice C → 8/8; Slice C → 5/8. Lossless links ⇒ pure channel-access/half-duplex contention (the s23 beacon-starvation class), not RF loss.
  - ★ **LBT ruled OUT as the driver:** `lbt_enabled=false` (both the wrapper gate and `FirmwareNode::_lbt_enabled` — same config key) leaves it **5/8, still starved**. So the sim's non-metal LBT model is NOT s19's cause, and **the Wave-4 LBT-energy slice will likely NOT rescue s19.** The residual `beacon_skipped_busy` is the R4.3 quiet-threshold throttle (`node_beacon.cpp:929`, firmware), not LBT.
- **s15 cross-layer ~90→~70% (worst seeds 1, 42 ≈ 5/21, 9/21) = gateway-window misalignment** — a DIFFERENT class. Beacons healthy (median rx≈242, not starved). Signature = `xl_handoff_giveup` + `xl_send_no_gateway` + `tx_gateway_schedule_defer ×2396`: cross-layer DMs miss the gateway's time-multiplexed layer window once Slice C decorrelates sender phases from the gateway windows. Failures are **silent** (0 giveups) — the documented s15 weakness, stressed harder.
- **Shared root (high-confidence hypothesis):** both are *"a stable route is never re-propagated to a node/leg that missed the initial advertisement window"* — s19 (a starved relay) and s15 (a time-multiplexed gateway leg). This is the **"dirty-only steady-state beacons never re-advertise stable routes"** class BASELINE already names for s15; the **reactive-pull / periodic re-advertisement** mechanism (already carries s15 per BASELINE) is the likely fix for **both** — NOT the LBT-energy model.
- **Ratification sequencing (refined):** don't ratify Slice C's delivery references now. The antidote is the **route-re-advertisement / reactive-pull resilience** work, re-measured under Slice C — *then* ratify. LBT-energy is still worth doing for metal-accuracy but is not the s19 fix (probe-proven). Next confirmatory step if wanted: force periodic full-table re-advertisement and check node-5 (s19) + the cross-layer legs (s15) recover.
