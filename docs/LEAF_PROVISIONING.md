...
A node finds and talks on a network using a small **manual floor**: *frequency + bandwidth + control SF + layer*. These can't be auto-distributed — you can't receive the config that tells you how to receive. **Everything operational beyond the floor** (data SFs, duty, leaf name) is **pulled and applied automatically** once a node hears its managed leaf.
...
> **A freshly-flashed node boots IDLE** — `layer = 0` means *unconfigured*, so it does **nothing on the air** (no DAD/J, no beacons, no REQ_SYNC) until your first `join`/`create`. Flash, then provision: it never transmits on the wrong default channel in between. `status` reads `unprovisioned` until then.

> `layer` is the user-facing 1..255 network id; the on-wire **leaf** nibble is `layer & 0x0F` (only 16 distinct leaves on the air, so two `layer`s with the same low nibble collide — `cfg` shows `layer=100 leaf=4` so you can spot a clash). Coding rate is a fixed low default (4/5) and isn't an argument — LoRa CRs interoperate.

## The three verbs

| goal | command |
|---|---|
| **join** an existing network | `join layer=<1..255> freq=<MHz> bw=<kHz> sf=<ctrl_sf>` |
| **create** a new managed leaf (this node = mother) | `create layer=<1..255> freq=<MHz> bw=<kHz> sf=<ctrl_sf> sf_list=<7,9> duty=<pct> name="<leaf name>" [active_fraction=] [ch_min_ms=] [dm_min_ms=]` |
...
```
join layer=2 freq=868.0 bw=250 sf=8
```
(Named `key=value` args — order-free — the same grammar as `gateway`. `layer` is the 1..255 network id; the on-wire leaf nibble is `layer & 0x0F`. `bw` is kHz and **may be fractional** — `bw=62.5` seeds 62500 Hz, also 41.67 / 31.25 / 20.83 … `duty` is a percent and **may be fractional too** — `duty=0.1` for a tight EU sub-band.)
- Sets the radio floor live (freq / bw / control SF / leaf nibble = `layer & 0x0F`), drops any old id, and re-DADs a fresh id (normal nodes pick **17..254**; 1..16 are reserved for gateways).
...
```
create layer=2 freq=868.0 bw=250 sf=8 sf_list=7,9 duty=10 name="north field"
```
...
- **Mints a managed lineage** (random, never 0) at **epoch 1** → this node becomes the leaf mother. Every node that `join`s the same `layer` later pulls *this* config.
...
```
leave
join layer=5 freq=869.4 bw=125 sf=7
```
...
| `status` · `cfg` · `whoami` | inspect id / layer / leaf / lineage / epoch / radio |
