...
- **Peer name → app:** a peer's name **rides the pubkey exchange** — it's appended to the WANT_PUBKEY H query and to all three pubkey-answer frames, and cached alongside the peer key (immutable key / mutable name, refreshed on each exchange). So after a `reqpubkey`/answer (or a QR import), the node holds the peer's name. **Query it with `nameof 0x<hash>`** → today **human text** (`[nameof] 0x<hash> = "<name>"`, `firmware_commands.cpp:401`), **not JSON**. **Firmware ask (PROPOSED → SPEC'D 2026-07-16):** surface the cached peer name to the app as JSON — decided shape (spec `2026-07-16-companion-mobile-team-json-surface.md` §7): **`peer_key_cached` gains `"name"` (omit-when-unknown)** + **`nameof` answers `{"ev":"peer_name","hash":…,"name":…}`** (a query answer, not a spontaneous push; `msg_recv` unchanged) — so a contact auto-labels in the same event that enables encrypted send. Until it lands the app keeps its own contact names (the QR `n`) and may `nameof` opportunistically.
...
## Mobile node + teams (mobile + teams 6.1–6.4 IMPLEMENTED/committed; team multi-hop routing parity landing; the STATE-JSON surface still mostly PROPOSED — 2026-07-16)
...
**★ Reality check for the app:** the **command/routing spine is live** (`send -t`, `reqpubkey -t`, team channel, team-DAD) but the **app-facing STATE JSON is still mostly PROPOSED** — verified against `console_json.cpp` today: **no mobile/team fields in `ready` or `status`**, **no `mobile_reg` push**, **no `team_id` tag on `channel_recv`/`inbox_channel`**. Only `cfg` exposes a `mobile` bool. So today the app can *send/receive* on the team plane but cannot yet *observe* mobile/team state as JSON — those are the firmware asks below, still owed.

> **★ 2026-07-16 — the missing JSON is now SPEC'D for the firmware coder:**
> `docs/superpowers/specs/2026-07-16-companion-mobile-team-json-surface.md` (slices S1–S6: `ready`/`cfg`
> mobile+team fields · `mobile_reg`/`team_reg` pushes · `mobile status`/`gateways` as JSON · live+durable
> `team_id` channel tag · peer names as JSON). **Where the spec differs from the sketches below, the spec
> wins** — notably: team ids ride as a **quoted hex string** (like `key`), NOT decimal; `ready` also gains
> **`team_local`** (our own team-overlay id — the sketches missed it) and uses **omit-when-inactive** (no
> `mobile:false`); `mobile gateways` is **streamed** (`mobile_gw`/`mobile_net` lines + `mobile_gw_end`,
> the `routes` pattern); the durable `inbox_channel` team tag bumps the store record version. Fold the
> final shapes into this contract when the firmware lands.
