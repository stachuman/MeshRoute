...
final class ConversationStoreTests: XCTestCase {
    ...
    func testInboundDMDedupByOriginCtr() {
        ...
        XCTAssertNotNil(store.ingest(.messageReceived(origin: 2, ctr: 5, senderHash: nil, seq: nil, body: "hi"), now: now))
        XCTAssertNil(store.ingest(.messageReceived(origin: 2, ctr: 5, senderHash: nil, seq: nil, body: "hi"), now: now)) // duplicate
        XCTAssertEqual(store.messages(in: .dm(KeyHash(2))).count, 1)
        // A different ctr from the same origin is a new message.
        XCTAssertNotNil(store.ingest(.messageReceived(origin: 2, ctr: 6, senderHash: nil, seq: nil, body: "again"), now: now))
    ...
    func testChannelIngest() {
        var store = ConversationStore()
        store.ingest(.channelReceived(origin: 4, channelID: 3, channelMsgID: 0x0400_0001, seq: nil, body: "gm"), now: now)
    ...
    func testRekeyDMFromIDToHash() {
        var store = ConversationStore()
        store.ingest(.messageReceived(origin: 2, ctr: 1, senderHash: nil, seq: nil, body: "hi"), now: now)   // legacy DM staged under dm(id)
    ...
    func testDMKeyedAndDedupedBySenderHash() {
        ...
        store.ingest(.messageReceived(origin: 2, ctr: 7, senderHash: h, seq: nil, body: "hi"), now: now)
        ...
        XCTAssertNil(store.ingest(.messageReceived(origin: 9, ctr: 7, senderHash: h, seq: nil, body: "hi"), now: now))
        XCTAssertEqual(store.messages(in: .dm(KeyHash(h))).count, 1)
    }
