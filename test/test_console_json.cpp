...
TEST_CASE("★ enum->string mappers cover EVERY enumerator — no silent fallback at the app boundary") {
    ...
    // so listing an enumerator in `ord()` (which the build now FORCES) automatically enrolls it here.
    // ⓘ WHY NOT A `_count` SENTINEL: adding one to `CmdCode` would put a non-value enumerator into every `switch` over
    // it, so `-Wswitch` would then demand a `case _count:` arm at each mapper — trading a silent under-cover for a
    // permanent tax on the instrument that is working. Deriving from the walker costs nothing and cannot drift.
    for (unsigned v = 0; v < 256u; ++v) {
