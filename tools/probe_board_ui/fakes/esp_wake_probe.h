...
//
// phA5 PROBE SHIM — the host stand-in for the two ESP-IDF calls §B197 uses to arm the user button as a light-sleep
// wake source. `driver/gpio.h` and `esp_sleep.h` (the two other files in this directory) both include THIS one, so the
// counters and the scripted return codes live in exactly one place rather than being duplicated per header (U1).
//
// ★ WHY IT HAS TO EXIST AT ALL: `variants/heltec_v3/board_ui.cpp` is compiled by neither the native suite nor the
//   simulator, so `enable_button_wake()` — a function whose ONLY observable behaviour is which platform calls it makes
//   and what it does with their return codes — would otherwise have no automated cover of any kind.
//
