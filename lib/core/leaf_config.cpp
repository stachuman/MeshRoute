...
uint16_t leaf_config_hash(uint16_t allowed_sf_bitmap, uint16_t duty_bp,
    ...
    uint8_t msg[1 + 2 + 2 + 2 + 2 + 1 + protocol::leaf_name_max];       // sf ‖ duty ‖ frac ‖ chI ‖ dmI ‖ len ‖ name (=20)
    size_t  n = 0;
    ...
    msg[n++] = static_cast<uint8_t>(active_fraction_bp);                // u16 active_fraction_bp LE (anti-spam v2)
    msg[n++] = static_cast<uint8_t>(active_fraction_bp >> 8);
    msg[n++] = static_cast<uint8_t>(ch_interval_ms);                    // u16 channel_min_interval_ms LE
    msg[n++] = static_cast<uint8_t>(ch_interval_ms >> 8);
    msg[n++] = static_cast<uint8_t>(dm_interval_ms);                    // u16 dm_min_interval_ms LE
    msg[n++] = static_cast<uint8_t>(dm_interval_ms >> 8);
...
// C config frame body codec (LE): sf_list (u8) · duty_bp (u16) · active_fraction_bp (u16) · ch_interval_ms (u16) ·
// dm_interval_ms (u16) · config_epoch (u16) · name_len (u8) · name (§2). Fixed prefix = 12 B.
size_t pack_c_config(const CConfig& in, uint8_t* out, size_t cap) {
    ...
    const size_t need = 12 + nlen;
    if (cap < need) return 0;
    size_t i = 0;
    out[i++] = sf_bitmap_to_wire(in.allowed_sf_bitmap);                 // u8 SF list
    out[i++] = uint8_t(in.duty_bp);              out[i++] = uint8_t(in.duty_bp >> 8);
    out[i++] = uint8_t(in.active_fraction_bp);   out[i++] = uint8_t(in.active_fraction_bp >> 8);
    out[i++] = uint8_t(in.ch_interval_ms);       out[i++] = uint8_t(in.ch_interval_ms >> 8);
    out[i++] = uint8_t(in.dm_interval_ms);       out[i++] = uint8_t(in.dm_interval_ms >> 8);
...
bool parse_c_config(const uint8_t* body, size_t len, CConfig& out) {
    ...
    out.active_fraction_bp = uint16_t(body[3]  | (body[4]  << 8));
    out.ch_interval_ms     = uint16_t(body[5]  | (body[6]  << 8));
    out.dm_interval_ms     = uint16_t(body[7]  | (body[8]  << 8));
    out.config_epoch       = uint16_t(body[9]  | (body[10] << 8));
    const uint8_t nlen = body[11];
    if (nlen > protocol::leaf_name_max || len < size_t(12) + nlen) return false;
    out.leaf_name_len = nlen;
    for (uint8_t k = 0; k < nlen; ++k) out.leaf_name[k] = char(body[12 + k]);
    return true;
}
