...
// [seq u32][kind u8][origin u8][channel_id u8][ctr u16][rx_time_ms u64][body_len u8][body] — all LE.
uint16_t serialize(uint8_t* out, uint32_t seq, InboxKind kind, uint8_t origin, uint8_t channel_id,
                   uint16_t ctr, uint64_t rx_time_ms, const uint8_t* body, uint8_t len) {
    uint8_t* p = out;
    w_u32(p, seq);
...
void Inbox::record(InboxStore* store, uint32_t& next, uint8_t& unpersisted, InboxKind kind, uint8_t origin,
    ...
    const uint16_t n = serialize(buf, seq, kind, origin, channel_id, ctr, now_ms, body, len);
...
void Inbox::record_dm(uint8_t origin, uint16_t ctr, const uint8_t* body, uint8_t len, uint64_t now_ms) {
    if (!enabled()) return;
    record(_dm, _dm_next, _dm_unpersisted, InboxKind::dm, origin, /*channel_id*/ 0, ctr, body, len, now_ms);
}

void Inbox::record_channel(uint8_t channel_id, uint8_t origin, uint16_t ctr,
                           const uint8_t* body, uint8_t len, uint64_t now_ms) {
    if (!enabled()) return;
    record(_chan, _chan_next, _chan_unpersisted, InboxKind::channel, origin, channel_id, ctr, body, len, now_ms);
}
