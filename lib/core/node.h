...
#include "team_seen_ring.h"  // §UI-16 N1: TeamSeen + the pure nearby-team observation ring (read-only; no other plane is touched)
...
public:
    ...
    // ★★★ §UI-16 N1 (spec 2026-08-22 §2.1) — THE NEARBY-TEAM OBSERVATION CACHE, READ SIDE.
    // Two `const`, allocation-free, bounded accessors over the RAM-only ring in team_seen_ring.h. The
    // retention window is applied HERE, at the read (protocol::team_seen_retain_ms), so a stale entry on
    // a silent radio can never linger readable and no sweep or timer id is needed.
    // ⓘ NO CONSUMER YET, AND THE ONE ARRIVING IS NAMED: §UI-16 slice N2 (PROVISION -> `JOIN TEAM` ->
    // NEARBY) projects each entry to `fingerprint · n/3 · age`. ⛔ N1 adds no screen, verb, push or byte.
    // ⛔ WHAT THESE DO NOT MEAN: an entry is evidence that SOMETHING NEARBY ADVERTISED that team id, and
    // nothing more — not a peer, not a route, not a membership, not a key. The id is public by design.
    uint8_t         team_seen_count() const {
        return team_seen_ring_live_count(_team_seen, _team_seen_n, _hal.now(), protocol::team_seen_retain_ms);
    }
    // The i-th RETAINED entry in first-observed order; nullptr = out of range (the normal end-of-list
    // answer, never a zero record a caller could render as a real observation — the team_channel_pub()
    // nullptr idiom).
    const TeamSeen* team_seen_at(uint8_t i) const {
        return team_seen_ring_live_at(_team_seen, _team_seen_n, i, _hal.now(), protocol::team_seen_retain_ms);
    }
#else
    ...
    // §UI-16 N1: inert on a static build (gateway_*, MR_FEAT_TEAM 0) — the ring MEMBERS are compiled out
    // too, so this build costs +0 RAM. A node with no team plane can never act on a nearby team, so the
    // honest answer is an EMPTY list rather than a pretend one (C2). The write site needs no #if: the
    // observe stub below swallows the call, exactly as the mint/adopt stubs do.
    uint8_t         team_seen_count() const { return 0; }
    const TeamSeen* team_seen_at(uint8_t) const { return nullptr; }
#endif
    ...
    // ★★★ §hybrid-rts S2 — the COMPLETED-FLIGHT cache (design §4.3). The match is the FULL tuple
    // `immediate sender | dst | team/static plane | domain | full identity` and nothing else; `_find` returns the
    // live entry or nullptr. `_store` prunes expired, refreshes an existing key in place, then inserts — evicting
    // the min-expiry entry only when every slot is live. Public for tests (like record_seen_origin above).
    // ⛔ `from` must be the ON-AIR immediate sender (`PendingRx::from` = the RTS's `src`), NEVER `meta.src_hint`.
    const CompletedFlight* completed_flight_find(uint8_t from, uint8_t dst, bool team_plane,
                                                 const RtsFlightIdentity& id, uint64_t now_ms) const;
    void    completed_flight_store(uint8_t from, uint8_t dst, bool team_plane,
                                   const RtsFlightIdentity& id, uint64_t now_ms);
    size_t  completed_flight_live_count(uint64_t now_ms) const;   // live (unexpired) occupancy — for tests/instrumentation
...
private:
    ...
    // ★★★ §hybrid-rts S2 — THE ONE PRODUCER OF THE WIRE ADDRESSING MARKS a unicast RTS carries for a pending
    // flight. Extracted from `tx_rts_retry` because S2 gained a SECOND consumer: the sender-side terminal-CTS echo
    // must compare the plane it DECLARED, and re-deriving `team_next` at that site would fork the §team-parity T8
    // predicate in two (U1). `pack` uses these verbatim; `rts_wire_team_plane(addr_len, mobile_src)` turns them
    // into the plane. ⛔ Do not add a third derivation of `team_next`.
    struct RtsWireMarks { uint8_t addr_len = 0; bool mobile_src = false; bool team_src = false; };
    RtsWireMarks rts_wire_marks(const PendingTx& pt) const;
    // ★★★ §hybrid-rts S2 — THE ONE derivation of a pending flight's identity (node_mac.cpp). Two consumers:
    // `tx_rts_retry` (what goes on the wire) and `handle_cts` (what a terminal echo must equal). ⛔ Do not inline
    // it at either: the first draft did, they diverged on `pt.origin` vs the DATA-exposed origin, and the corpus
    // answered with 40 terminal-CTS refusals and a retry deadlock.
    RtsFlightIdentity flight_identity(const PendingTx& pt) const;
...
private:
    ...
    // ★★★ §UI-16 N1 (spec 2026-08-22 §2.1) — THE NEARBY-TEAM OBSERVATION CACHE, WRITE SIDE + STORAGE.
    // Node-GLOBAL, not LayerRuntime, and `_peer_loc`'s reasoning is reused verbatim: a team_id is a
    // layer-INDEPENDENT identity, so a team heard on one leaf is equally audible-as-a-fact on the other
    // — per-leaf copies would cost 2x the RAM to hold two answers to a question that has one.
    // ★★ RAM ONLY, DELIBERATELY VOLATILE, NO NV RECORD — and unlike `_peer_loc` that needs no argument:
    // an observation is a statement about RIGHT NOW on THIS radio, so a reboot-restored one would be
    // meaningless by construction. `team_seen_retain_ms` is a 10-minute window; a boot outlives it.
    // ⛔ #if MR_FEAT_TEAM: the cache is team-plane-only by construction (a gateway can never act on it),
    // so the two members compile out on the gateway_* envs and the cost there is +0.
    // ★ THE COUNT BYTE IS DECLARED BEFORE THE ARRAY, AND THAT IS MEASURED, NOT ARGUED: here it lands in
    // the alignment pad that already sat between `_parked_sends_n` and the 8-aligned `_l2c_redirect`, so
    // it costs ZERO — sizeof(Node) is identical with and without it (probed by removal). Declared AFTER
    // the 8-aligned array it measures +8, because it would then open a fresh hole of its own.
#if MR_FEAT_TEAM
    uint8_t    _team_seen_n = 0;
    TeamSeen   _team_seen[protocol::cap_team_seen] = {};
    // The ONE write path. Called from exactly one site — `ingest_beacon`, immediately after the
    // leaf/team drop — and it touches ONLY `_team_seen`/`_team_seen_n`. ⛔ No route merge, no
    // `team_key_set`, no `id_bind_set`, no `peer_key_set`, no NV, no TX, no timer, no MR_EMIT.
    void team_seen_observe(uint32_t team_id, int16_t sample_q4, uint8_t src_id) {
        team_seen_ring_observe(_team_seen, _team_seen_n, team_id, sample_q4, src_id, _hal.now());
    }
#else
    void team_seen_observe(uint32_t, int16_t, uint8_t) {}   // §featuresplit: inert -> the one call site stays unchanged
#endif
    ...
    struct LayerRuntime {
        ...
        // ★★★ §hybrid-rts S2 (2026-08-08) — THE COMPLETED-FLIGHT CACHE, and it is NOT `_last_acked_from` restored:
        // it is keyed by the FULL identity the 10/11-B RTS now carries (see `CompletedFlight` in node_carriers.h for
        // the key and why `payload_len` is absent), it stores the WIRE-declared plane, and its TTL is DERIVED
        // (`completed_flight_cache_ttl_ms` = `gateway_send_giveup_ms`), not the retired 10 s.
        // ★ A FIXED PER-LAYER ARRAY, not a map: no heap, deterministic eviction, and bounded by construction —
        //   the plan's "smallest bounded deterministic cache; do not allocate an unbounded/per-peer dynamic
        //   structure". Empty slot == `expiry_ms == 0`. Full table: prune expired, then evict min-expiry (U1 —
        //   `record_seen_origin`'s policy verbatim).
        // ⚠ PER-LAYER IS NOT PLANE ISOLATION (design §4.3): a gateway's two leaves get separate tables, but the
        //   TEAM and STATIC planes COEXIST inside one layer, which is why `team_plane` is part of the key.
        CompletedFlight _completed_flights[protocol::cap_completed_flights] = {};
