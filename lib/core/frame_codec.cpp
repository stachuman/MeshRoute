...
size_t pack_j_offer(const j_offer_in& in, std::span<uint8_t> out) {
    if (out.size() < (in.is_mobile ? 13u : 8u)) return 0;        // §mobile 2a/S6: a mobile OFFER is 13 B (+proposed_mobile_id +target_key_hash32)
    wire::Writer w(out);
    w.u8(j_b0(in.leaf_id));
    ...
    if (in.is_mobile) { w.u8(in.proposed_mobile_id);            // §mobile 2a: the host-assigned LOCAL id
                        w.u32_le(in.target_key_hash32); }      // §S6: the mobile this OFFER is addressed to (13-B mobile OFFER)
    return w.ok() ? w.size() : 0;
}
    ...
    switch (static_cast<j_opcode>(o.opcode)) {
        ...
        case j_opcode::offer:
            ...
            if (o.is_mobile) { o.proposed_mobile_id   = r.u8();   // §mobile 2a: host-assigned LOCAL id
                               o.target_key_hash32 = r.u32_le(); }  // §S6: the mobile this OFFER is addressed to
            break;
        case j_opcode::claim:
